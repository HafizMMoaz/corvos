    ...
    *,
    ...
    if len(parts) != 2 or not event.external_peer_id:
        await adapter.send_message(
            ...
            text="Generate a pairing code in Corvos Settings > Messaging Channels, then send /start CODE here.",
        )
        return True
    ...
    if binding is None:
        await adapter.send_message(
            external_peer_id=event.external_peer_id,
            text="That pairing code is invalid or expired. Generate a new code in Corvos.",
        )
        return True

    await adapter.send_message(
        external_peer_id=event.external_peer_id,
        text="Corvos is connected. Send a message here to chat with your agent.",
    )
    return True
...
async def send_unbound_onboarding(
    *,
    ...
    await adapter.send_message(
        external_peer_id=event.external_peer_id,
        text=(
            "Hi! To use Corvos via Telegram, generate a pairing code at "
