...
async def _resolve_mention_context(
    ...
    *,
...
) -> tuple[str, CorvosContextSchema | None]:
    ...
    build a ``CorvosContextSchema`` that the ``search_knowledge_base`` tool
    ...
    # ``CorvosContextSchema.mentioned_connectors`` is typed ``list[dict]`` and
    ...
    runtime_context = CorvosContextSchema(
        workspace_id=workspace_id,
...
async def run_agent_task(
    *,
    ...
    async with async_session_maker() as agent_session:
        ...
        # The compiled graph declares ``context_schema=CorvosContextSchema``;
        ...
        while True:
            ...
            config["configurable"]["corvos_resume_value"] = routed
