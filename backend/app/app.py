...
from app.exceptions import GENERIC_5XX_MESSAGE, ISSUES_URL, CorvosError
...
_error_logger = logging.getLogger("corvos.errors")

rate_limit_logger = logging.getLogger("corvos.rate_limit")
...
def _corvos_error_handler(request: Request, exc: CorvosError) -> JSONResponse:
...
def _check_rate_limit(
    ...
    key = f"corvos:auth_rate_limit:{scope}:{client_ip}"
...
def _enable_slow_callback_logging(threshold_sec: float = 0.5) -> None:
    ...
    _slow_log = logging.getLogger("corvos.perf.slow")
...
async def _warm_agent_jit_caches() -> None:
    ...
    * Bounded by ``asyncio.wait_for`` so a hang here can never block
    ...
    try:
        ...
        # ``create_corvos_deep_agent`` (1.5-2s of CPU per call to
        ...
        compiled = create_agent(
            ...
            context_schema=CorvosContextSchema,
            checkpointer=None,
        )
...
async def _apply_admin_setting_overrides() -> None:
    """Apply every stored super-admin settings-vault override against the
    live `Config` singleton (setattr shadow trick) at boot, so an override
    set before a restart survives the restart. Non-fatal: a failure here
    just means overrides set via the admin dashboard silently fall back to
    their `Config` class defaults until the next successful boot or the
    next `PUT /admin/settings/{key}` call (which re-applies live anyway).
    """
    logger = logging.getLogger(__name__)
    try:
        from app.db import async_session_maker
        from app.services.settings_vault_service import load_all_overrides_and_apply

        async with async_session_maker() as session:
            await load_all_overrides_and_apply(session)
    except Exception:
        logger.warning(
            "[startup] Applying admin settings-vault overrides failed (non-fatal)",
            exc_info=True,
        )


async def _apply_admin_llm_catalog() -> None:
    """Apply the DB-backed admin LLM provider/model catalog
    (`admin_llm_providers`/`admin_llm_models`) against the live
    `config.GLOBAL_LLM_CONFIGS` singleton at boot, so admin-managed models
    are available immediately without waiting on the first catalog
    mutation. Runs after `_apply_admin_setting_overrides` -- same ordering
    rationale: DB connectivity must be confirmed first. Non-fatal: a
    failure here just means admin-managed models are absent from
    `GLOBAL_LLM_CONFIGS` until the next successful boot or catalog mutation
    (which re-applies live anyway).
    """
    logger = logging.getLogger(__name__)
    try:
        from app.db import async_session_maker
        ...
        logger.warning(
            "[startup] Applying admin LLM catalog failed (non-fatal)",
            exc_info=True,
        )


async def _apply_admin_connector_credential_overrides() -> None:
    """Apply every stored super-admin connector-credentials-vault override
    against the live `Config` singleton (setattr shadow trick) at boot, so
    an override set before a restart survives the restart. Runs after
    `_apply_admin_setting_overrides` and `_apply_admin_llm_catalog` -- same
    ordering rationale: DB connectivity must be confirmed first. Non-fatal:
    a failure here just means overrides set via the admin dashboard
    silently fall back to their `Config` class defaults until the next
    successful boot or the next `PUT /admin/connector-credentials/{key}`
    call (which re-applies live anyway).
    """
    logger = logging.getLogger(__name__)
    try:
        from app.db import async_session_maker
        from app.services.admin_connector_credentials_service import (
            load_all_connector_credential_overrides_and_apply,
        )

        async with async_session_maker() as session:
            await load_all_connector_credential_overrides_and_apply(session)
    except Exception:
        logger.warning(
            "[startup] Applying admin connector-credentials-vault overrides "
            "failed (non-fatal)",
            exc_info=True,
        )
...
async def lifespan(app: FastAPI):
    ...
    await _apply_admin_setting_overrides()
    await _apply_admin_llm_catalog()
    await _apply_admin_connector_credential_overrides()
...
app.add_exception_handler(CorvosError, _corvos_error_handler)
    ...
    async def dispatch(
        ...
        if elapsed_ms > _PERF_SLOW_REQUEST_THRESHOLD:
            with contextlib.suppress(Exception):
                ...
                span.set_attribute("corvos.request.elapsed_ms", elapsed_ms)
    ...
    async def dispatch(
        ...
        with contextlib.suppress(Exception):
            ...
            if ctx is not None and ctx.method == "pat" and ph_analytics.is_enabled():
                ...
                client = (
                    "mcp"
                    if request.headers.get("X-Corvos-Client") == "mcp"
                    else "pat_script"
                )
...
if config.AUTH_TYPE == "GOOGLE":
    ...
    from app.users import get_google_oauth_client
    ...
    async def _google_authorization_url(request: Request, response: Response) -> str:
        ...
        # Built per request off the live `config` singleton so the client_id
        # baked into the authorization URL always matches the one the token
        # exchange and id_token audience check below read -- an admin
        # connector-credentials-vault override must not desync the two.
        authorization_url = await get_google_oauth_client().get_authorization_url(
