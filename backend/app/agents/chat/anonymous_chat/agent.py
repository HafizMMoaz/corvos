...
deliberately bypass the full Corvos deep agent stack (filesystem,
...
For any other Corvos capability (including web search) the model is
...
def build_anonymous_system_prompt(anon_doc: dict[str, Any] | None = None) -> str:
    ...
    a registration funnel: every other Corvos capability (scraping, live
    ...
    return (
        "You are Corvos's free AI assistant, available to everyone without "
        "login. Corvos is the open-source open web research platform: "
        ...
        "briefly say that this needs a free Corvos account and invite them "
        "to sign up at https://www.corvos.com/register - name the specific "
        "capability that would solve their request (e.g. 'a Corvos account "
        ...
        "## Suggesting Corvos when it genuinely fits\n"
        "You are also the front door to Corvos. When the conversation "
        ...
        "that a free Corvos account can do that with live data, linking "
        "https://www.corvos.com/register.\n"
        ...
        "- Do not suggest it for needs Corvos does not serve (casual chat, "
...
async def create_anonymous_chat_agent(
    *,
    ...
    Unlike :func:`create_corvos_deep_agent`, this agent has no filesystem,
    ...
    middleware: list[Any] = [
        ...
        create_corvos_compaction_middleware(llm, StateBackend),
    ...
    return agent.with_config(
        {
            "recursion_limit": 40,
            "metadata": {
                "ls_integration": "corvos_anonymous_chat",
