...
Flag key:    ``corvos:spawn_paused:{workspace_id}``
...
# by setting ``CORVOS_TASK_SPAWN_PAUSED_DISABLED=1``. Default is
# enabled so production never relies on flipping an opt-out flag.
_DISABLED = os.environ.get(
    "CORVOS_TASK_SPAWN_PAUSED_DISABLED", ""
).strip().lower() in {
    "1",
    "true",
    "yes",
...
def _flag_key(workspace_id: int) -> str:
    return f"corvos:spawn_paused:{workspace_id}"
