...
``create_corvos_deep_agent`` runs a 4-5 second pipeline on EVERY chat
turn:
...
Most middleware in the Corvos stack captures per-thread state in
...
Phase 2 will move those captured fields onto :class:`CorvosContextSchema`
...
Every lookup logs ``[agent_cache]`` lines through ``corvos.perf``:
