...
``wrap_tool_call`` / their async counterparts), so a Corvos-specific
...
factory callable under the ``corvos.plugins`` entry-point group:

.. code-block:: toml

    # in a plugin package's pyproject.toml
    [project.entry-points."corvos.plugins"]
...
PLUGIN_ENTRY_POINT_GROUP = "corvos.plugins"
...
def load_allowed_plugin_names_from_env() -> set[str]:
    """Read ``CORVOS_ALLOWED_PLUGINS`` (comma-separated) into a set.
    ...
    entries are dropped.
    """
    raw = os.environ.get("CORVOS_ALLOWED_PLUGINS", "").strip()
    if not raw:
        return set()
