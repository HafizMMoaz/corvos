...
def build_main_agent_system_prompt(
    *,
    ...
    if custom_system_instructions and custom_system_instructions.strip():
        # Literal placeholder substitution, not str.format(): this text is
        # free-form user input (workspace custom instructions) and may contain
        # unrelated "{"/"}" characters that would raise on .format().
        parts.append(
            "\n"
            + custom_system_instructions.replace("{resolved_today}", resolved_today)
            + "\n"
        )
