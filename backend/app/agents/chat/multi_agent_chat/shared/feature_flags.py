...
OFF. The master kill-switch ``CORVOS_DISABLE_NEW_AGENT_STACK`` forces every
...
class AgentFeatureFlags:
    ...
    # CORVOS_ENABLE_AGENT_CACHE=false.
    ...
    # Rollback via CORVOS_ENABLE_CROSS_THREAD_AGENT_CACHE=false.
    ...
    def from_env(cls) -> AgentFeatureFlags:
        ...
        force to False.
        """
        master_off = _env_bool("CORVOS_DISABLE_NEW_AGENT_STACK", False)
        if master_off:
            logger.info(
                "CORVOS_DISABLE_NEW_AGENT_STACK is set: every new agent "
        ...
        return cls(
            ...
            # Agent quality
            enable_context_editing=_env_bool("CORVOS_ENABLE_CONTEXT_EDITING", True),
            enable_compaction_v2=_env_bool("CORVOS_ENABLE_COMPACTION_V2", True),
            enable_retry_after=_env_bool("CORVOS_ENABLE_RETRY_AFTER", True),
            enable_model_fallback=_env_bool("CORVOS_ENABLE_MODEL_FALLBACK", False),
            enable_model_call_limit=_env_bool(
                "CORVOS_ENABLE_MODEL_CALL_LIMIT", True
            ),
            enable_tool_call_limit=_env_bool("CORVOS_ENABLE_TOOL_CALL_LIMIT", True),
            enable_tool_call_repair=_env_bool(
                "CORVOS_ENABLE_TOOL_CALL_REPAIR", True
            ),
            enable_doom_loop=_env_bool("CORVOS_ENABLE_DOOM_LOOP", True),
            # Safety
            enable_permission=_env_bool("CORVOS_ENABLE_PERMISSION", True),
            enable_busy_mutex=_env_bool("CORVOS_ENABLE_BUSY_MUTEX", True),
            enable_llm_tool_selector=_env_bool(
                "CORVOS_ENABLE_LLM_TOOL_SELECTOR", False
            ),
            # Skills + subagents
            enable_skills=_env_bool("CORVOS_ENABLE_SKILLS", True),
            enable_specialized_subagents=_env_bool(
                "CORVOS_ENABLE_SPECIALIZED_SUBAGENTS", True
            ),
            # Snapshot / revert
            enable_action_log=_env_bool("CORVOS_ENABLE_ACTION_LOG", True),
            enable_revert_route=_env_bool("CORVOS_ENABLE_REVERT_ROUTE", True),
            # Plugins
            enable_plugin_loader=_env_bool("CORVOS_ENABLE_PLUGIN_LOADER", False),
            # Observability
            enable_otel=_env_bool("CORVOS_ENABLE_OTEL", False),
            # Performance
            enable_agent_cache=_env_bool("CORVOS_ENABLE_AGENT_CACHE", True),
            enable_cross_thread_agent_cache=_env_bool(
                "CORVOS_ENABLE_CROSS_THREAD_AGENT_CACHE", True
            ),
            enable_agent_cache_share_gp_subagent=_env_bool(
                "CORVOS_ENABLE_AGENT_CACHE_SHARE_GP_SUBAGENT", False
            ),
        )
