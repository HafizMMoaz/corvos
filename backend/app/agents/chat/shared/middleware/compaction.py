...
1. A structured summary template (:data:`CORVOS_SUMMARY_PROMPT`) instead of
...
CORVOS_SUMMARY_PROMPT = """<role>
Corvos Conversation Compaction Assistant
</role>
...
## Relevant Files
What documents or paths in the Corvos knowledge base are in play? Use ``/documents/...`` paths exactly as they appeared in the workspace tree.
</instructions>
...
class CorvosCompactionMiddleware(SummarizationMiddleware):
    """SummarizationMiddleware tuned for Corvos.
...
def create_corvos_compaction_middleware(
    model: BaseChatModel,
    ...
    **overrides: Any,
    ...
    """Build a :class:`CorvosCompactionMiddleware` with sensible defaults.
    ...
    Args:
        ...
        summary_prompt: Optional override; defaults to :data:`CORVOS_SUMMARY_PROMPT`.
        history_path_prefix: Path prefix for offloaded conversation history.
        **overrides: Forwarded to :class:`CorvosCompactionMiddleware`.
    """
    defaults = compute_summarization_defaults(model)
    return CorvosCompactionMiddleware(
        model=model,
        backend=backend,
        ...
        summary_prompt=summary_prompt or CORVOS_SUMMARY_PROMPT,
