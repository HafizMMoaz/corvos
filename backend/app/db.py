...
INCENTIVE_TASKS_CONFIG = {
    IncentiveTaskType.GITHUB_STAR: {
        ...
        "description": "Show your support by starring Corvos on GitHub",
        # Credit reward in USD micro-units (1_000_000 == $1.00). $0.03.
        "credit_micros_reward": 30000,
        "action_url": "https://github.com/HafizMMoaz/Corvos",
    },
    IncentiveTaskType.REDDIT_FOLLOW: {
        "title": "Join our Subreddit",
        "description": "Join the Corvos community on Reddit",
        "credit_micros_reward": 30000,
        "action_url": "https://www.reddit.com/r/Corvos/",
    },
    IncentiveTaskType.DISCORD_JOIN: {
        "title": "Join our Discord",
        "description": "Join the Corvos community on Discord",
        "credit_micros_reward": 40000,
        "action_url": "https://discord.gg/Ggf9PxDNQ2",
    },
    # Future tasks can be configured here:
    # IncentiveTaskType.GITHUB_ISSUE: {
    #     "title": "Create an issue",
    #     "description": "Help improve Corvos by reporting bugs or suggesting features",
    #     "credit_micros_reward": 50000,
    #     "action_url": "https://github.com/HafizMMoaz/Corvos/issues/new/choose",
    # },
}
...
class NewChatThread(BaseModel, TimestampMixin):
    ...
    # Surface metadata for first-party Corvos and external chat threads.
    # Zero publishes all chat-message sources; the UI can decide which surfaces to render.
    source = Column(Text, nullable=False, default="corvos", server_default="corvos")
...
class PlatformPermission(StrEnum):
    """
    Granular permissions for the platform-wide super admin dashboard.
    Distinct from `Permission`, which is scoped to a single workspace.
    Use '*' (FULL_ACCESS) to grant all admin permissions.
    """

    SETTINGS_READ = "settings:read"
    SETTINGS_WRITE = "settings:write"
    SETTINGS_REVEAL = "settings:reveal"

    LLM_PROVIDERS_READ = "llm_providers:read"
    LLM_PROVIDERS_WRITE = "llm_providers:write"

    QUOTAS_READ = "quotas:read"
    QUOTAS_WRITE = "quotas:write"
    ...
    ADMIN_ROLES_WRITE = "admin_roles:write"
...
PLATFORM_SYSTEM_ROLES: dict[str, list[str]] = {
    "super_admin": [PlatformPermission.FULL_ACCESS.value],
    "billing_admin": [
    ...
    "support_admin": [
        PlatformPermission.USERS_READ.value,
        PlatformPermission.USERS_WRITE.value,
...
class PlatformRole(BaseModel, TimestampMixin):
    """
    Platform-wide admin roles (distinct from per-workspace WorkspaceRole).
    Assigning a user one of these roles grants access to the super admin
    dashboard with the associated permission set.
    """
    ...
    description = Column(String(500), nullable=True)
    # List of PlatformPermission enum values (e.g., ["billing:read", "billing:write"])
    permissions = Column(ARRAY(String), nullable=False, default=[])
    # System roles (super_admin, billing_admin, support_admin) cannot be deleted
    is_system_role = Column(Boolean, nullable=False, default=False)

    assignments = relationship(
        "PlatformRoleAssignment", back_populates="role", passive_deletes=True
    )


class PlatformRoleAssignment(BaseModel, TimestampMixin):
    """
    Grants a user a platform-wide admin role. Presence of any assignment
    row for a user is what makes them an admin at all.
    """
    ...
    role = relationship("PlatformRole", back_populates="assignments")
    assigned_by = relationship("User", foreign_keys=[assigned_by_id])


class AdminAuditLog(BaseModel):
    """
    Append-only log of every mutation made through the super admin dashboard.
    Every admin route that changes state writes exactly one row here via
    `app.utils.platform_rbac.record_admin_action`.
    """
...
class AdminSetting(BaseModel, TimestampMixin):
    """
    One row per live-overridden `Config` attribute (or synthetic setting
    key), managed through the super admin settings vault
    (`app.services.settings_vault_service`). Absence of a row for a given
    `key` means the `Config` class default is in effect -- rows are only
    created when an admin sets a non-default value, and deleted on revert.

    Secret values are encrypted at rest in `value_encrypted` via the same
    `TokenEncryption` class already used for OAuth tokens; non-secret values
    are stored as plain text in `value_plain`. Exactly one of the two is
    populated, based on `is_secret`.
    """
    ...
    is_disruptive = Column(Boolean, nullable=False, default=False)
...
class AdminConnectorCredential(BaseModel, TimestampMixin):
    """
    One admin-configured OAuth app credential pair for a connector
    integration (Slack, Notion, Google, etc.), managed through the super
    admin connectors vault (`app.services.admin_connector_credentials_service`).
    `connector_key` is validated against the curated
    `app.config.connector_credentials_registry.CONNECTOR_CREDENTIALS_REGISTRY`
    at write time -- presence of a row means an override of the `Config`
    class default is in effect for that connector's client_id/client_secret
    `Config` attributes; absence means the class (env-derived) default
    applies. `is_enabled=False` suspends a stored override live without
    deleting the encrypted secret, distinct from a full revert (row
    deletion).

    `client_id_encrypted`/`client_secret_encrypted` are encrypted at rest via
    the same `TokenEncryption` class already used for OAuth tokens and
    Phase B/C's other admin-managed secrets.
    """

    __tablename__ = "admin_connector_credentials"

    connector_key = Column(String(50), nullable=False, unique=True, index=True)
...
class AdminLLMProvider(BaseModel, TimestampMixin):
    """
    One admin-configured LLM provider connection (API credentials + base
    transport behavior), managed through the super admin LLM catalog
    (`app.services.admin_llm_catalog_service`). `provider_key` is free-text,
    not FK'd to any enum -- it is looked up against the static
    `app.services.provider_registry.REGISTRY` at apply time, with an unknown
    key falling back to a synthetic native spec (see `spec_for()`).

    `api_key_encrypted` is encrypted at rest via the same `TokenEncryption`
    class already used for OAuth tokens and Phase B's settings vault.
    """
    ...
    transport = Column(String(30), nullable=False)
    litellm_prefix = Column(String(50), nullable=True)
    default_base_url = Column(String(500), nullable=True)
    base_url_required = Column(Boolean, nullable=False, default=False)
    auth_style = Column(String(20), nullable=False)
    api_key_encrypted = Column(Text, nullable=True)
    api_base_override = Column(String(500), nullable=True)
    is_enabled = Column(Boolean, nullable=False, default=True)
    ...
    updated_by = relationship("User", foreign_keys=[updated_by_id])
    models = relationship(
        "AdminLLMModel", back_populates="provider", passive_deletes=True
    )


class AdminLLMModel(BaseModel, TimestampMixin):
    """
    One admin-configured LLM model exposed to workspaces, managed through the
    super admin LLM catalog. `id` is this row's own DB primary key -- it is
    NOT the synthetic negative id used when this row is mapped into
    `config.GLOBAL_LLM_CONFIGS` (see `app.services.admin_llm_catalog_service`,
    which derives that id as `-(100000 + id)`).
    """

    __tablename__ = "admin_llm_models"
    __table_args__ = (
        UniqueConstraint(
            "provider_id", "model_name", name="uq_admin_llm_model_provider_model"
        ),
    )

    provider_id = Column(
        Integer,
        ForeignKey("admin_llm_providers.id", ondelete="CASCADE"),
        nullable=False,
    )
    name = Column(String(200), nullable=False)
    model_name = Column(String(300), nullable=False)
    billing_tier = Column(String(20), nullable=False, default="premium")
    anonymous_enabled = Column(Boolean, nullable=False, default=False)
    seo_enabled = Column(Boolean, nullable=False, default=False)
    seo_slug = Column(String(200), nullable=True, unique=True)
    seo_title = Column(String(300), nullable=True)
    seo_description = Column(Text, nullable=True)
    quota_reserve_tokens = Column(Integer, nullable=True)
    ...
    litellm_params = Column(JSONB, nullable=True)
    ...
    is_planner = Column(Boolean, nullable=False, default=False)
    router_pool_eligible = Column(Boolean, nullable=False, default=True)
    is_enabled = Column(Boolean, nullable=False, default=True)
    created_by_id = Column(
        UUID(as_uuid=True), ForeignKey("user.id", ondelete="SET NULL"), nullable=True
    )
    ...
    provider = relationship("AdminLLMProvider", back_populates="models")
    created_by = relationship("User", foreign_keys=[created_by_id])
    updated_by = relationship("User", foreign_keys=[updated_by_id])


class FreeModelGlobalQuota(BaseModel):
    """
    Single-row (by convention, id=1) global token quota for free-tier LLM
    usage across the whole deployment for the current period. Enforcement
    lives in the free-model quota track built on top of this table; this
    class only declares the schema created by migration 177.
    """

    __tablename__ = "free_model_global_quota"

    period_start = Column(Date, nullable=False)
    tokens_reserved = Column(BigInteger, nullable=False, default=0)
...
class FreeModelUserQuota(BaseModel):
    """
    Per-user token quota for free-tier LLM usage for the current period (one
    row per user). Enforcement lives in the free-model quota track built on
    top of this table; this class only declares the schema created by
    migration 177.
    """

    __tablename__ = "free_model_user_quota"
...
class Plan(BaseModel, TimestampMixin):
    """
    One subscription plan (Free/Pro/Team-style), managed through the super
    admin billing dashboard (`app.services.entitlement_service`). A plan's
    two Paddle-facing columns (`monthly_credit_micros`, `paddle_price_id`)
    are populated here but only ever read/acted on by the Paddle billing
    track: `monthly_credit_micros` is the USD micro-unit amount
    (`1_000_000 == $1.00`, matching `User.credit_micros_balance`) granted on
    each successful subscription renewal, and `paddle_price_id` is set once
    an admin creates the matching Paddle price. `is_active=False` hides a
    plan from new signups without deleting it or breaking the `User.plan_id`
    FK of members already on it.

    No `users` back-reference on purpose -- nothing in this track needs to
    load "all users on plan X" through the ORM relationship graph; a future
    consumer queries `User.plan_id == plan.id` directly instead.
    """
    ...
    description = Column(Text, nullable=True)
    monthly_credit_micros = Column(BigInteger, nullable=False, default=0)
    paddle_price_id = Column(String(255), nullable=True, unique=True)
    is_active = Column(Boolean, nullable=False, default=True)
    updated_by_id = Column(
        UUID(as_uuid=True), ForeignKey("user.id", ondelete="SET NULL"), nullable=True
    )
...
class FeatureFlag(BaseModel, TimestampMixin):
    """
    One feature flag definition, managed through the super admin billing
    dashboard. A flag's per-plan default lives in `PlanFeatureValue`, and a
    per-user forced value lives in `UserFeatureOverride` -- this table only
    declares the flag itself (key, display name, description).
    """
...
class PlanFeatureValue(BaseModel, TimestampMixin):
    """
    One plan's configured value for one feature flag. Absence of a row for a
    given `(plan_id, feature_flag_id)` pair means that flag is **disabled**
    for that plan by default -- rows only exist once an admin actively
    enables (or explicitly disables) a flag for a plan. See
    `app.services.entitlement_service` for the full resolution order
    (plan default, then any unexpired `UserFeatureOverride`).
    """

    __tablename__ = "plan_feature_values"
    __table_args__ = (
        UniqueConstraint("plan_id", "feature_flag_id", name="uq_plan_feature_value"),
    )
...
class UserFeatureOverride(BaseModel, TimestampMixin):
    """
    One user's forced value for one feature flag, granted by an admin --
    overrides the flag's plan default in either direction (force-on or
    force-off) regardless of what the user's plan says. `expires_at = NULL`
    means the override never expires; an expired override is left in place
    (not deleted) so its history remains visible, but
    `app.services.entitlement_service` stops honoring it once expired and
...
class PlanModelEntitlement(BaseModel, TimestampMixin):
    """
    One model a plan is allowed to use. `config_id` is the synthetic
    cross-source model identifier already used everywhere else in the
    codebase (`AgentConfig.config_id` in
    `app.agents.chat.runtime.llm_config`, spanning YAML-static negative ids,
    OpenRouter ids, admin-DB-catalog ids via `-(100000 + id)`, and positive
    BYOK ids) -- not a DB foreign key, since no single table backs all of
    those id spaces.

    A plan with **zero** rows here allows every model (unrestricted); rows
    only start restricting once an admin actively adds them. See
    `app.services.entitlement_service` for the resolution logic.
    """

    __tablename__ = "plan_model_entitlements"
    __table_args__ = (
        UniqueConstraint("plan_id", "config_id", name="uq_plan_model_entitlement"),
    )
...
class PaddleCustomer(BaseModel, TimestampMixin):
    """One Paddle Customer per user (Phase E, Track 2 -- Paddle billing).

    Created lazily on the user's first subscription checkout via
    `app.services.paddle_service.get_or_create_paddle_customer`.
    ``paddle_customer_id`` is an opaque-but-not-sensitive reference (format
    ``ctm_...``), stored in plaintext exactly like
...
class PaddleSubscription(BaseModel, TimestampMixin):
    """One Paddle subscription for a user (Phase E, Track 2 -- Paddle billing).

    Upserted by `app.services.paddle_service.upsert_subscription_from_webhook`
    on `subscription.created` / `subscription.updated` / `subscription.canceled`.
    ``status`` stores Paddle's raw status string verbatim
    (`active`/`trialing`/`past_due`/`paused`/`canceled`) -- Paddle is the
    source of truth for what states exist. ``plan_id`` is resolved from the
    subscription's price id and may be NULL if that price id doesn't match
    any known plan (e.g. an admin renamed/removed the plan after the price
    was created in Paddle); the row is still stored so the subscription
    ...
    paddle_customer_id = Column(String(255), nullable=False, index=True)
    ...
    user = relationship("User", back_populates="paddle_subscriptions")
    plan = relationship("Plan")


class PaddleTransaction(BaseModel, TimestampMixin):
    """Tracks Paddle transactions used to grant subscription credit
    (Phase E, Track 2 -- Paddle billing).

    Mirrors `CreditPurchase`'s idempotent webhook-fulfillment shape exactly:
    ``paddle_transaction_id`` is the idempotency key (mirrors
    `CreditPurchase.stripe_checkout_session_id`'s role), and ``status``
    reuses the existing `CreditPurchaseStatus` enum rather than defining a
    new one. ``credit_micros_granted`` stores the USD-micro amount added to
    ``user.credit_micros_balance`` on fulfillment.
    """
    ...
    paddle_transaction_id = Column(String(255), nullable=False, unique=True, index=True)
    ...
    credit_micros_granted = Column(BigInteger, nullable=False, default=0)
    amount_total = Column(Integer, nullable=True)
    currency = Column(String(10), nullable=True)
    completed_at = Column(TIMESTAMP(timezone=True), nullable=True)

    user = relationship("User", back_populates="paddle_transactions")
    plan = relationship("Plan")
...
if config.AUTH_TYPE == "GOOGLE":
    ...
    class User(SQLAlchemyBaseUserTableUUID, Base):
        ...
        # Platform-wide admin role assignments (super admin dashboard)
        platform_role_assignments = relationship(
        ...
        paddle_subscriptions = relationship(
            "PaddleSubscription",
            back_populates="user",
            cascade="all, delete-orphan",
        )
        paddle_transactions = relationship(
            "PaddleTransaction",
            back_populates="user",
            cascade="all, delete-orphan",
        )
        ...
        # Subscription plan (super admin billing dashboard). NULL means no
        # plan assigned -- entitlement_service treats that as unrestricted
        # model access and every feature flag defaulting to disabled.
        plan_id = Column(
            Integer,
