...
if config.AUTH_TYPE == "GOOGLE":
    ...
    def get_google_oauth_client() -> GoogleOAuth2:
        """Build a Google OAuth client from the *current* live config values.

        Deliberately constructed per call instead of once at import time.
        The admin connector-credentials vault can override
        `GOOGLE_OAUTH_CLIENT_ID`/`GOOGLE_OAUTH_CLIENT_SECRET` live via
        `setattr` on the `config` singleton, and an import-time singleton
        would keep serving the value captured before that override existed
        (module import happens before the FastAPI lifespan hook that loads
        the vault, so it can never see an override at all). The token
        exchange and the id_token audience check in `app.app` both read
        `config.GOOGLE_OAUTH_CLIENT_ID`/`_SECRET` live, so a stale
        authorization-URL client_id would split the two halves of the same
        sign-in and break every Google login until the next restart.
        """
        return GoogleOAuth2(
...
async def get_auth_context(
    ...
    receives the full Corvos principal instead of a bare User.
    """
