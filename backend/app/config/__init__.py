...
os.environ.setdefault("OR_APP_NAME", "Corvos")
os.environ.setdefault("OR_SITE_URL", "https://corvos.com")
...
class Config:
    ...
    if not is_ffmpeg_installed():
        allow_static_ffmpeg = (
            os.getenv("CORVOS_ALLOW_STATIC_FFMPEG_DOWNLOAD", "TRUE").upper()
            == "TRUE"
        )
        ...
        if not is_ffmpeg_installed():
            raise ValueError(
                "FFmpeg is not installed on the system. Please install it to use the Corvos Podcaster."
            )
    ...
    DEPLOYMENT_MODE = os.getenv("CORVOS_DEPLOYMENT_MODE", "self-hosted")
    ...
    CELERY_TASK_DEFAULT_QUEUE = os.getenv("CELERY_TASK_DEFAULT_QUEUE", "corvos")
    ...
    PADDLE_RECONCILIATION_INTERVAL = os.getenv("PADDLE_RECONCILIATION_INTERVAL", "10m")
    ...
    AGENT_CACHE_MAXSIZE = int(os.getenv("CORVOS_AGENT_CACHE_MAXSIZE", "256"))
    AGENT_CACHE_TTL_SECONDS = float(
        os.getenv("CORVOS_AGENT_CACHE_TTL_SECONDS", "1800")
    )

    # Connector discovery cache TTL
    CONNECTOR_DISCOVERY_TTL_SECONDS = float(
        os.getenv("CORVOS_CONNECTOR_DISCOVERY_TTL_SECONDS", "30")
    )

    CORVOS_PUBLIC_URL = os.getenv("CORVOS_PUBLIC_URL")
    NEXT_FRONTEND_URL = os.getenv("NEXT_FRONTEND_URL") or CORVOS_PUBLIC_URL
    # Backend URL to override the http to https in the OAuth redirect URI
    BACKEND_URL = os.getenv("BACKEND_URL") or CORVOS_PUBLIC_URL
    ...
    # Paddle billing (subscription checkout for plans; additive to Stripe's
    # one-off credit-pack purchases above, not a replacement for it).
    PADDLE_API_KEY = os.getenv("PADDLE_API_KEY")
    PADDLE_WEBHOOK_SECRET = os.getenv("PADDLE_WEBHOOK_SECRET")
    # Not sent on any current Paddle Billing REST API call (confirmed against
    # the Create Transaction / Create Customer docs) -- kept for
    # forward-compatibility / operator reference only.
    PADDLE_VENDOR_ID = os.getenv("PADDLE_VENDOR_ID")
    PADDLE_ENVIRONMENT = os.getenv("PADDLE_ENVIRONMENT", "sandbox")
    PADDLE_RECONCILIATION_LOOKBACK_MINUTES = int(
        os.getenv("PADDLE_RECONCILIATION_LOOKBACK_MINUTES", "10")
    )
    PADDLE_RECONCILIATION_BATCH_SIZE = int(
        os.getenv("PADDLE_RECONCILIATION_BATCH_SIZE", "100")
    )
    ...
    # Free-tier model quota caps (monthly, token-counted). See
    # app.services.free_model_quota_service for the reserve/finalize/release
    # enforcement built on top of these.
    FREE_MODEL_GLOBAL_MONTHLY_CAP_TOKENS = int(
        os.getenv("FREE_MODEL_GLOBAL_MONTHLY_CAP_TOKENS", "50000000")
    )
    FREE_MODEL_USER_MONTHLY_CAP_TOKENS = int(
        os.getenv("FREE_MODEL_USER_MONTHLY_CAP_TOKENS", "500000")
    )
    ...
    SESSION_COOKIE_NAME = os.getenv("SESSION_COOKIE_NAME", "corvos_session")
    REFRESH_COOKIE_NAME = os.getenv("REFRESH_COOKIE_NAME", "corvos_refresh")
