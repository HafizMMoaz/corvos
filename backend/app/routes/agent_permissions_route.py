...
The route group is gated by ``CORVOS_ENABLE_PERMISSION``: when off
    ...
    if flags.disable_new_agent_stack or not flags.enable_permission:
        raise HTTPException(
            ...
            detail=(
                ...
                "Flip CORVOS_ENABLE_PERMISSION to enable them."
            ),
        )
...
async def list_rules(
    ...
    _flag_guard()
    await _ensure_workspace_membership_admin(session, auth, workspace_id)
