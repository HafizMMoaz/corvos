...
async def add_manual_model(
    ...
    """Add one model by id without discovery, for providers whose /models
    endpoint is missing, broken, or gated (e.g. some OpenAI-compatible
    routers/proxies)."""
    conn = await _load_connection(session, connection_id)
    await _assert_connection_access(
        session, auth, conn, Permission.LLM_CONFIGS_CREATE.value
    ...
    existing = next((m for m in conn.models if m.model_id == model_id), None)
    if existing is not None:
        existing.enabled = True
        await session.commit()
        await session.refresh(existing)
        return _model_read(existing)

    selection = data.model_copy(
        update={"model_id": model_id, "source": ModelSource.MANUAL, "enabled": True}
    )
