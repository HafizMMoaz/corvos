...
building blocks under ``flows/shared/``. Mirrors ``stream_new_chat`` but:
  ...
  * No mentions / corvos-doc / report context assembly (seeded by original).
...
async def stream_resume_chat(
    ...
    free_reservation: FreeQuotaReservation | None = None
    ...
    try:
        ...
        if needs_free_quota(agent_config, user_id):
            free_reservation = await reserve_free_quota(
                agent_config=agent_config,
                user_id=user_id,  # type: ignore[arg-type]
            )
            if not free_reservation.allowed:
                ot.add_event("quota.denied", {"quota.code": "FREE_QUOTA_EXHAUSTED"})
                yield emit_stream_error(
                    message=(
                        "The free tier's monthly usage limit has been reached. "
                        "Try again next month, or switch to a premium model."
                    ),
                    error_kind="free_quota_exhausted",
        ...
        config = {
            "configurable": {
                ...
                # ``CorvosCheckpointedSubAgentMiddleware``. Parallel
                # siblings each pop their own entry, so they never race.
                "corvos_resume_value": routing.routed_resume_value,
            },
        ...
        if free_reservation is not None and user_id:
        ...
        with anyio.CancelScope(shield=True):
            ...
            if free_reservation is not None and user_id:
                await release_free_quota(reservation=free_reservation, user_id=user_id)
