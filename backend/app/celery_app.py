...
def _stamp_enqueue_time(headers=None, **_kwargs):
    ...
    with contextlib.suppress(Exception):
        headers["corvos.enqueued_at_ns"] = str(time.monotonic_ns())
...
def _record_queue_latency(task=None, **_kwargs):
    ...
    try:
        ...
        with contextlib.suppress(Exception):
            request.corvos_operation = operation
            request.corvos_queue = queue
            request.corvos_scheduled = scheduled

        headers = getattr(request, "headers", None) or {}
        enqueued_ns = headers.get("corvos.enqueued_at_ns")
        if enqueued_ns is None:
            return

        elapsed_s = (time.monotonic_ns() - int(enqueued_ns)) / 1e9
        with contextlib.suppress(Exception):
            request.corvos_queue_latency_ms = elapsed_s * 1000
...
def _set_celery_span_attributes(task=None, **_kwargs):
    ...
    try:
        ...
        operation = getattr(request, "corvos_operation", None)
        if operation:
            span.set_attribute("celery.task.operation", operation)

        latency_ms = getattr(request, "corvos_queue_latency_ms", None)
...
async def _apply_admin_vault_overrides() -> None:
    """Apply both super-admin vaults' stored overrides against this process's
    live `Config` singleton.

    The FastAPI process does exactly this at startup via
    `app.app._apply_admin_setting_overrides` and
    `app.app._apply_admin_connector_credential_overrides`. A Celery worker is
    a separate process with its own `Config` singleton, so without the same
    load here a worker keeps reading the `Config` class defaults forever.
    That silently breaks background work for any attribute an admin moved
    into a vault and removed from `.env` -- e.g. the Dropbox and OneDrive
    background token refreshes reached from the connector indexing tasks read
    `config.DROPBOX_APP_KEY` / `config.MICROSOFT_CLIENT_ID` straight off the
    singleton.

    The two vaults are loaded independently and non-fatally, mirroring
    `app.app`'s two startup helpers: one vault failing must not stop the
    other from applying, and neither failure may stop the worker from
    booting (the overrides simply stay unapplied in this process until it is
    restarted, or until an admin re-saves them, which live-applies in the
    ...
    try:
        ...
        logger.warning(
            "[celery] Applying admin settings-vault overrides failed (non-fatal)",
            exc_info=True,
        )
    ...
    try:
        ...
        logger.warning(
            "[celery] Applying admin connector-credentials-vault overrides "
            "failed (non-fatal)",
            exc_info=True,
        )
...
def init_worker(**kwargs):
    ...
    Also loads both admin vaults' live overrides so this worker's `Config`
    singleton matches the FastAPI process's - see
    `_apply_admin_vault_overrides`.
    """
    from app.observability.bootstrap import init_otel

    init_otel(app=None, traces=True, metrics=True, logs=True)

    # Runs before the routers are built so an admin-vault override of an
    # LLM-related setting is already live when they read config. Uses the
    # canonical celery async bridge rather than a bare `asyncio.run` so the
    # shared engine's pool never hands a connection bound to this
    # now-closed loop to a later task.
    from app.tasks.celery_tasks import run_async_celery_task

    run_async_celery_task(_apply_admin_vault_overrides)
...
PADDLE_RECONCILIATION_INTERVAL = config.PADDLE_RECONCILIATION_INTERVAL
...
paddle_reconciliation_schedule_params = parse_schedule_interval(
    PADDLE_RECONCILIATION_INTERVAL
)
...
celery_app = Celery(
    ...
    include=[
        ...
        "app.tasks.celery_tasks.paddle_reconciliation_task",
        "app.tasks.celery_tasks.refresh_token_cleanup_task",
        "app.tasks.celery_tasks.free_model_quota_reset_task",
...
celery_app.conf.beat_schedule = {
    ...
    # Reconcile Paddle transactions that were paid but remained pending
    "reconcile-pending-paddle-transactions": {
        "task": "reconcile_pending_paddle_transactions",
        "schedule": crontab(**paddle_reconciliation_schedule_params),
        "options": {
            "expires": 60,
        },
    },
    ...
    "reset-free-model-quotas": {
        "task": "reset_free_model_quotas",
