...
_INSTRUMENTATION_NAME = "corvos.platform"
...
def _package_version() -> str:
    ...
    with contextlib.suppress(Exception):
        return metadata.version("corvos-backend")
    return "unknown"
...
def _model_call_duration():
    return _get_meter().create_histogram(
        "corvos.model.call.duration",
        unit="ms",
        description="Duration of Corvos LLM model calls.",
    )
...
def _tool_call_duration():
    return _get_meter().create_histogram(
        "corvos.tool.call.duration",
        unit="ms",
        description="Duration of Corvos agent tool calls.",
    )
...
def _tool_call_errors():
    return _get_meter().create_counter(
        "corvos.tool.call.errors",
        description="Count of Corvos agent tool call errors.",
    )
...
def _kb_search_duration():
    return _get_meter().create_histogram(
        "corvos.kb.search.duration",
        unit="ms",
        description="Duration of Corvos knowledge-base search calls.",
    )
...
def _compaction_runs():
    return _get_meter().create_counter(
        "corvos.compaction.runs",
        description="Count of Corvos conversation compaction runs.",
    )
...
def _permission_asks():
    return _get_meter().create_counter(
        "corvos.permission.asks",
        description="Count of Corvos permission asks.",
    )
...
def _interrupts():
    return _get_meter().create_counter(
        "corvos.interrupt.raised",
        description="Count of Corvos interrupts raised.",
    )
...
def _indexing_document_duration():
    return _get_meter().create_histogram(
        "corvos.indexing.document.duration",
        unit="s",
        description="Duration of Corvos document indexing.",
    )
...
def _indexing_document_outcome():
    return _get_meter().create_counter(
        "corvos.indexing.document.outcome",
        description="Count of Corvos document indexing outcomes.",
    )
...
def _connector_sync_duration():
    return _get_meter().create_histogram(
        "corvos.connector.sync.duration",
        unit="s",
        description="Duration of Corvos connector sync tasks.",
    )
...
def _connector_sync_outcome():
    return _get_meter().create_counter(
        "corvos.connector.sync.outcome",
        description="Count of Corvos connector sync outcomes.",
    )
...
def _auth_failures():
    return _get_meter().create_counter(
        "corvos.auth.failures",
        description="Count of Corvos authentication failures.",
    )
...
def _rate_limit_rejections():
    return _get_meter().create_counter(
        "corvos.rate_limit.rejections",
        description="Count of Corvos rate-limit rejections.",
    )
...
def _perf_elapsed():
    return _get_meter().create_histogram(
        "corvos.perf.elapsed_ms",
        unit="ms",
        description="Elapsed time recorded by Corvos perf timers.",
    )
...
def _chat_request_duration():
    return _get_meter().create_histogram(
        "corvos.chat.request.duration",
        unit="ms",
        description="Duration of Corvos streamed chat requests.",
    )
...
def _chat_request_outcome():
    return _get_meter().create_counter(
        "corvos.chat.request.outcome",
        description="Count of Corvos chat request outcomes.",
    )
...
def _subagent_invoke_duration():
    return _get_meter().create_histogram(
        "corvos.subagent.invoke.duration",
        unit="ms",
        description="Duration of Corvos subagent invocations.",
    )
...
def _subagent_invoke_outcome():
    return _get_meter().create_counter(
        "corvos.subagent.invoke.outcome",
        description="Count of Corvos subagent invocation outcomes.",
    )
...
def _etl_extract_duration():
    return _get_meter().create_histogram(
        "corvos.etl.extract.duration",
        unit="s",
        description="Duration of Corvos ETL extraction.",
    )
...
def _etl_extract_outcome():
    return _get_meter().create_counter(
        "corvos.etl.extract.outcome",
        description="Count of Corvos ETL extraction outcomes.",
    )
...
def _embedding_cache_lookups():
    return _get_meter().create_counter(
        "corvos.embedding.cache.lookups",
...
def _embedding_cache_evictions():
    return _get_meter().create_counter(
        "corvos.embedding.cache.evictions",
...
def _chunk_reconcile_chunks():
    return _get_meter().create_counter(
        "corvos.indexing.reconcile.chunks",
        description=(
...
def _celery_heartbeat_refreshes():
    return _get_meter().create_counter(
        "corvos.celery.heartbeat.refreshes",
        description="Count of Corvos Celery heartbeat refreshes.",
    )
...
def _celery_heartbeat_failures():
    return _get_meter().create_counter(
        "corvos.celery.heartbeat.failures",
        description="Count of Corvos Celery heartbeat failures.",
    )
...
def _celery_queue_latency():
    return _get_meter().create_histogram(
        "corvos.celery.queue.latency",
        unit="s",
        description="Time Corvos Celery tasks spend waiting in queue.",
    )
...
def _gateway_thread_lock_contention():
    return _get_meter().create_counter(
        "corvos.gateway.thread_lock.contention",
...
def _gateway_inbox_processed():
    return _get_meter().create_counter(
        "corvos.gateway.inbox.processed",
...
def _gateway_inbound_reconciled():
    return _get_meter().create_counter(
        "corvos.gateway.inbound.reconciled",
...
def _gateway_rate_limit_hits():
    return _get_meter().create_counter(
        "corvos.gateway.rate_limit.hits",
...
def _gateway_health_check_failures():
    return _get_meter().create_counter(
        "corvos.gateway.health_check.failures",
...
def _gateway_auth_invariant_failures():
    return _get_meter().create_counter(
        "corvos.gateway.auth_invariant.failures",
...
def _gateway_active_bindings():
    return _get_meter().create_up_down_counter(
        "corvos.gateway.active_bindings",
...
def register_runtime_observables() -> None:
    ...
    try:
        ...
        meter.create_observable_gauge(
            ...
            unit="By",
            description="Resident set size of the Corvos backend process.",
        )
        meter.create_observable_gauge(
            ...
            unit="1",
            description="CPU utilization of the Corvos backend process.",
        )
        meter.create_observable_gauge(
            "process.runtime.cpython.threads",
            callbacks=[lambda _options: _runtime_snapshot_value("threads")],
            unit="{thread}",
            description="Thread count of the Corvos backend process.",
        )
        meter.create_observable_gauge(
            "process.runtime.cpython.open_fds",
            callbacks=[lambda _options: _runtime_snapshot_value("open_fds")],
            unit="{fd}",
            description="Open file descriptor count of the Corvos backend process.",
        )
        meter.create_observable_gauge(
