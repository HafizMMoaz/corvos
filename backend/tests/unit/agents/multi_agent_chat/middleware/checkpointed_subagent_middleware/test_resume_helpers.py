...
class TestConsumeCorvosResume:
    def test_pops_only_entry_matching_runtime_tool_call_id(self):
        ...
        assert consume_corvos_resume(runtime) == {"decisions": ["approve"]}
    ...
    def test_popping_one_entry_leaves_siblings_untouched(self):
        ...
        consume_corvos_resume(runtime_a)

        assert configurable["corvos_resume_value"] == {
            "tcid-B": {"decisions": ["reject"]}
        }
    ...
    def test_returns_none_when_no_entry_for_this_tool_call(self):
        runtime = _runtime_with_config(
            {
                "configurable": {
                    "corvos_resume_value": {"tcid-other": {"decisions": []}}
                }
            },
            tool_call_id="tcid-A",
        )

        assert consume_corvos_resume(runtime) is None
    ...
    def test_drops_empty_dict_after_last_entry_consumed(self):
        configurable = {
            "corvos_resume_value": {"tcid-A": {"decisions": ["approve"]}}
        }
        runtime = _runtime_with_config(
            {"configurable": configurable}, tool_call_id="tcid-A"
        )

        consume_corvos_resume(runtime)

        assert "corvos_resume_value" not in configurable


class TestHasCorvosResume:
    def test_true_when_entry_for_this_tool_call_present(self):
        ...
        assert has_corvos_resume(runtime) is True
    ...
    def test_false_when_entry_for_other_tool_call_only(self):
        ...
        assert has_corvos_resume(runtime) is False
