...
``PermissionMiddleware`` both used the Corvos-specific
...
async def test_parallel_self_gated_and_middleware_gated_route_and_resume_cleanly():
    ...
    # the unification. If either one regresses to the legacy Corvos shape
    ...
    config["configurable"]["corvos_resume_value"] = by_tool_call_id
