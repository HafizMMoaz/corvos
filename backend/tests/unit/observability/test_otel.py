...
def _reset_otel_state(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("CORVOS_DISABLE_OTEL", "true")
...
def test_enabled_when_endpoint_configured(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("CORVOS_DISABLE_OTEL", raising=False)
...
class TestBootstrapConfig:
    ...
    def test_resource_defaults_include_service_metadata(
        ...
        monkeypatch.setenv("CORVOS_ENV", "test")
    ...
    def test_deployment_environment_uses_corvos_env_only(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.delenv("CORVOS_ENV", raising=False)

        assert bootstrap._deployment_environment() == "dev"

        monkeypatch.setenv("CORVOS_ENV", "production")
...
class TestPackageVersionResilience:
    ...
    def test_package_not_found_falls_back(
        ...
        def _raise_not_found(_name: str) -> str:
            raise metrics.metadata.PackageNotFoundError("corvos-backend")
