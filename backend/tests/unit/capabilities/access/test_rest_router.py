...
def _register_corvos_handler(app: FastAPI) -> None:
    """Minimal stand-in for the app's global CorvosError handler."""
    from starlette.responses import JSONResponse

    from app.exceptions import CorvosError

    async def _handler(_request, exc: CorvosError):
        return JSONResponse(
            status_code=exc.status_code,
            content={"code": exc.code, "message": exc.message},
        )

    app.add_exception_handler(CorvosError, _handler)


@pytest.mark.asyncio
async def test_executor_fault_becomes_502(monkeypatch):
    """Any non-Corvos executor error is surfaced as a clean 502, not a 500."""
...
@pytest.mark.asyncio
async def test_corvos_error_passes_through(monkeypatch):
