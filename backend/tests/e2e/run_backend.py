...
- `backend/.dockerignore` excludes `tests/`, so this file
...
# Make the backend root importable as a top-level package so
...
logger = logging.getLogger("corvos.e2e.backend")
...
def _load_dotenv_and_set_env_defaults() -> None:
    ...
    os.environ.setdefault(
        "DATABASE_URL",
        "postgresql+asyncpg://postgres:postgres@localhost:5432/corvos",
    )
    os.environ.setdefault("CELERY_BROKER_URL", "redis://localhost:6379/0")
    os.environ.setdefault("CELERY_RESULT_BACKEND", "redis://localhost:6379/0")
    os.environ.setdefault("REDIS_APP_URL", "redis://localhost:6379/0")
    os.environ.setdefault("CELERY_TASK_DEFAULT_QUEUE", "corvos")
...
def _bootstrap():
    ...
    logger.warning(
        "*** CORVOS E2E BACKEND ENTRYPOINT - fake Composio + LLM + embeddings ***"
    )
