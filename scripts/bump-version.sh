...
bump_json "$REPO_ROOT/web/package.json"
bump_json "$REPO_ROOT/browser_extension/package.json"
bump_json "$REPO_ROOT/desktop/package.json"
bump_toml "$REPO_ROOT/backend/pyproject.toml"

echo ""
echo "Syncing lock files..."
if command -v uv &>/dev/null; then
  (cd "$REPO_ROOT/backend" && uv lock)
  echo "  OK    backend/uv.lock"
else
  echo "  SKIP  uv not found -- run 'uv lock' in backend/ manually"
fi
