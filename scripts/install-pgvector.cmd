...
set "LOG=e:\corvos\scripts\pgvector_install.txt"
set "PGROOT=C:\Program Files\PostgreSQL\18"
set "SRC=%TEMP%\pgvector"
set "PSQL=%PGROOT%\bin\psql.exe"
set "PGPASSWORD=corvos"
...
"%PSQL%" -w -U corvos -h 127.0.0.1 -p 5432 -d corvos -c "ALTER SYSTEM SET wal_level = 'logical';" >> "%LOG%" 2>&1
...
>> "%LOG%" echo skipped - database already created by create-corvos-role.cmd

>> "%LOG%" echo --- enabling extensions in corvos ---
"%PSQL%" -w -U corvos -h 127.0.0.1 -p 5432 -d corvos -c "CREATE EXTENSION IF NOT EXISTS vector; CREATE EXTENSION IF NOT EXISTS pg_trgm;" >> "%LOG%" 2>&1

>> "%LOG%" echo --- verification ---
"%PSQL%" -w -U corvos -h 127.0.0.1 -p 5432 -d corvos -At -c "select 'wal_level=' || current_setting('wal_level');" >> "%LOG%" 2>&1
"%PSQL%" -w -U corvos -h 127.0.0.1 -p 5432 -d corvos -At -c "select 'installed=' || extname || ' ' || extversion from pg_extension where extname in ('vector','pg_trgm');" >> "%LOG%" 2>&1
"%PSQL%" -w -U corvos -h 127.0.0.1 -p 5432 -d corvos -At -c "select 'roundtrip=' || ('[1,2,3]'::vector <-> '[1,2,4]'::vector)::text;" >> "%LOG%" 2>&1
