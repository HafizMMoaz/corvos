...
Bump-Json (Join-Path $RepoRoot "web\package.json")
Bump-Json (Join-Path $RepoRoot "browser_extension\package.json")
Bump-Json (Join-Path $RepoRoot "desktop\package.json")
Bump-Toml (Join-Path $RepoRoot "backend\pyproject.toml")
...
if (Get-Command uv -ErrorAction SilentlyContinue) {
    ...
    uv lock
    Pop-Location
    Write-Host "  OK    backend/uv.lock"
} else {
    Write-Host "  SKIP  uv not found -- run 'uv lock' in backend/ manually"
}
