...
def _client_recording(seen: dict) -> CorvosClient:
...
def _call_get_document(client: CorvosClient, **arguments) -> str:
    mcp = FastMCP("test")
    search_tools.register(mcp, client, MagicMock())
    blocks = asyncio.run(mcp.call_tool("corvos_get_document", arguments))
