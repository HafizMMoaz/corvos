...
def main() -> None:
    ...
    transport = os.environ.get("CORVOS_MCP_TRANSPORT", "stdio").strip() or "stdio"
    ...
    if transport == "stdio" and not settings.api_key:
        raise SystemExit(
            "CORVOS_API_KEY is required for stdio transport. Create an API "
            "key in Corvos (Settings -> API) and pass it via the "
            "CORVOS_API_KEY environment variable."
        )
