...
_FAILURE_HINTS: dict[int, str] = {
    401: "Authentication failed - the Corvos API key is invalid or expired.",
    402: "The workspace is out of credits for this operation.",
    403: (
        "Access denied - the token lacks permission, or API access is disabled "
        "for this workspace (enable it in Corvos workspace settings)."
    ),
...
class CorvosClient:
    ...
    def __init__(
        ...
        self._http = httpx.AsyncClient(
            base_url=api_base,
            # ``X-Corvos-Client`` lets the backend distinguish PAT traffic
            ...
            headers={"Accept": "application/json", "X-Corvos-Client": "mcp"},
            timeout=timeout,
        )
    ...
    def _auth_headers(self) -> dict[str, str]:
        ...
        if not api_key:
            raise ToolError(
                "No Corvos API key supplied. Send it as an 'Authorization: "
                "Bearer ss_pat_...' header (remote server), or set the "
                "CORVOS_API_KEY environment variable (stdio)."
            )
    ...
    async def request(
        ...
        *,
        ...
        try:
            ...
            raise ToolError(
                f"Could not reach Corvos at {self._api_base}: {exc}. "
                "Confirm the backend is running and CORVOS_BASE_URL is correct."
            ) from exc
    ...
    def _explain_failure(cls, response: httpx.Response) -> str:
        ...
        if detail:
            return f"Corvos returned {response.status_code}: {detail}"
        return hint or f"Corvos returned HTTP {response.status_code}."
