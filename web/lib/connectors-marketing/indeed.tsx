...
export const indeed: ConnectorPageContent = {
	...
	metaTitle: "Indeed API for Job Postings & Hiring Data | Corvos",
	metaDescription:
		"Scrape public Indeed job postings with the Corvos Indeed API: titles, companies, salaries, and full descriptions by search query or company page. Start free.",
	keywords: [
		"indeed api",
		"indeed scraper",
		"indeed scraper api",
	...
	heroLede:
		"The Corvos Indeed API is an Indeed scraper that extracts public job postings, salaries, companies, and full descriptions by search query, company page, or job URL - no partner approval required. Give your AI agents a live feed of who is hiring, for what, at what pay, so you track the labor market as it moves.",
	...
	comparison: {
		...
		intro:
			"Indeed retired its public Publisher jobs API and gates data behind partner programs. If you cannot get access or need clean structured jobs now, here is how Corvos compares.",
		columnLabel: "DIY Indeed scraping",
		rows: [
			{
				feature: "Access",
				official: "Publisher API retired; partner-gated and approval-only",
				corvos: "One API key; scrape public postings without an approval process",
			},
			{
				feature: "Anti-bot",
				official: "You fight Cloudflare, fingerprinting, and CAPTCHAs yourself",
				corvos: "Warmed, rotated sessions managed for you; no proxy plumbing",
			},
			{
				feature: "Pricing",
				official: "Proxy, browser, and maintenance costs you own",
				corvos: "Pay per job returned, with a free tier to start",
			},
			{
				feature: "Descriptions",
				official: "Extra page fetch and parsing you build and maintain",
				corvos: "Full description per job with one scrape_job_details flag",
			},
			{
				feature: "Agent-ready",
				official: "No; you build the harness yourself",
				corvos: "MCP server exposes indeed.scrape as a native tool",
			},
		],
	},
	...
	faq: [
		{
			...
			answer:
				"Corvos reads only public Indeed job postings, the same listings any logged-out visitor can see. It never logs in and cannot access private or applicant data. As always, review Indeed's terms and your own compliance needs before you run at scale.",
		},
		{
			question: "Does Indeed have an API?",
			answer:
				"Indeed retired its public Publisher jobs API and now gates job data behind partner and approval programs. Corvos is an independent Indeed API alternative: you call one API, or add the MCP server to your agent, and get structured public postings back.",
		},
		{
			question: "How much does the Indeed API cost?",
			answer:
				"Corvos prices per job returned, not per request or seat, and there is a free tier to start - no partner application or Indeed API pricing negotiation. You control spend with max_items per call, and jobs that fail to return are never billed.",
		},
		{
			question: "Can I get the full job description?",
			answer:
				"Yes. By default each job returns the listing snippet, which is fast. Set scrape_job_details to true and Corvos fetches each job's detail page for the full description text and HTML, at the cost of one extra page load per job.",
		},
		{
			question: "What are the rate limits?",
			answer:
				"Each call returns up to 100 jobs across all sources, with up to 20 URLs or search queries per request. Corvos manages the anti-bot request budget for you, so you scale reads without running proxies or a headless browser yourself.",
		},
	],
	...
	related: [
		...
		{ label: "Corvos MCP Server", href: "/mcp-server" },
	],
};
