...
export const amazon: ConnectorPageContent = {
	...
	metaTitle: "Amazon Product API for Price and Review Data | Corvos",
	metaDescription:
	...
	comparison: {
		...
		intro:
			"Amazon's official Product Advertising API requires an approved Associates account with qualifying sales, and most third-party Amazon data APIs bill per request, meter add-ons separately, and leave the discovery-to-detail logic to you. Here is how Corvos compares.",
		columnLabel: "Typical Amazon data API",
		rows: [
			{
				feature: "Pricing",
				official: "Per-request pricing that climbs fast at scale",
				corvos: "Pay per product returned, with a free tier to start",
			},
			{
				feature: "Discovery",
				official: "Separate search and detail endpoints you stitch together",
				corvos: "One verb takes search terms, product, category, or best-seller URLs",
			},
			{
				feature: "Offers & sellers",
				official: "Often separate paid endpoints",
				corvos: "Offers and seller profiles enriched inline on the product",
			},
			{
				feature: "Data access",
				official: "Requires accounts, keys, or approval for many providers",
				corvos: "Public, anonymous data only - no login or seller account",
			},
			{
				feature: "Agent-ready",
				official: "No; you wire the harness yourself",
				corvos: "MCP server exposes amazon.scrape as a native tool",
			},
		],
	},
	...
	faq: [
		{
			...
			answer:
				"An Amazon product API returns a listing's data as structured JSON instead of raw HTML. The Corvos Amazon API scrapes public product pages and gives you price, rating, review breakdown, offers, sellers, and best-seller ranks as clean JSON your agents can read.",
		},
		{
		...
		},
		{
			...
			answer:
				"Yes, for public catalog data. Amazon's Product Advertising API requires an approved Associates account with ongoing qualifying sales and is built for affiliate linking. Corvos needs no Amazon account: it scrapes public listings and returns prices, ratings, offers, sellers, and ranks as JSON.",
		},
		{
			question: "Which Amazon marketplaces are supported?",
			answer:
				"Marketplace routing is inferred from the Amazon URL or domain. Corvos supports public Amazon data from US, UK, Germany, Italy, and Spain. France is wired in and retried like the others, but remains best-effort because Amazon France is more sensitive to proxy-pool WAF challenges.",
		},
	],
