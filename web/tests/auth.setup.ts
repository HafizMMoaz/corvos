 ...
 *   - `corvos_announcements_state` - the blocking AnnouncementSpotlight
 *     dialog (e.g. "Introducing AI Automations") plus its toasts.
 *   - `corvos-tour-<userId>` - the OnboardingTour spotlight for new users.
 */
...
const SESSION_COOKIE_NAME = process.env.SESSION_COOKIE_NAME || "corvos_session";
const ANNOUNCEMENTS_KEY = "corvos_announcements_state";
...
setup("authenticate", async ({ page, request }) => {
	...
	await page.addInitScript(
		({ announcementsKey, state, uid }) => {
			...
			if (uid) {
				localStorage.setItem(`corvos-tour-${uid}`, "true");
			}
		},
		{
