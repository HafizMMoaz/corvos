...
1. `backend/tests/e2e/run_backend.py` and `run_celery.py` are
...
**2. Start the backend** in `backend/`, terminal A:
...
**3. Start the Celery worker** in `backend/`, terminal B:
...
curl -X POST http://localhost:8000/auth/register \
  ...
  -d '{"email":"e2e-test@corvos.net","password":"E2eTestPassword123!"}'
```

**5. Run Playwright** from `web/`, terminal C:
...
1. **Backend fake.** Add a new file under
   `backend/tests/e2e/fakes/<sdk>_module.py` mirroring
...
3. **Backend tests.** Put edge cases in backend tests, not Playwright:
   ...
   `backend/tests/integration/<connector>/`.
...
`backend/tests/unit/utils/test_oauth_security.py` and
`backend/tests/integration/composio/`.
