---
title: Ollama
description: Connect Ollama to Corvos
---
...
Choose the URL from where the Corvos backend runs.

### Corvos Runs in Docker

Use this when Corvos is running from Docker and Ollama is running on your computer.
...
<Callout type="info">
This is the default in Corvos.
</Callout>

### Ollama Runs in Docker

Use this only when Ollama is a service in the same Compose stack as Corvos.

```text
http://ollama:11434
```

### Corvos Runs Without Docker

Use this when Corvos and Ollama both run directly on the same computer.
...
Do not add `/v1` to the URL. Corvos uses Ollama native routes such as `/api/version` and `/api/tags`.
...
From the Corvos backend container:
...
Then refresh model discovery in Corvos.
