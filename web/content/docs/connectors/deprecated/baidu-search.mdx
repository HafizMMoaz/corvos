...
The Baidu Search connector is **deprecated** and can no longer be connected (the backend refuses new connections with HTTP 410). Public web search in Corvos now runs through the built-in **Google Search** specialist - see [Web Search](/docs/how-to/web-search). Existing Baidu connector rows remain manageable, and you can add an alternative provider through a Custom MCP connector. The guide below is retained for reference only.
</Callout>

This guide walks you through connecting Baidu AI Search to Corvos for Chinese web search and AI-powered research.

## How it works

The Baidu Search connector uses Baidu AI Search through Qianfan AppBuilder's intelligent search generation API. It is a live search connector: Corvos queries Baidu when the assistant needs current web results instead of periodically indexing content into your knowledge base.

- Baidu Search is best for Simplified Chinese queries and China-focused web content.
- Results are merged with Corvos's other configured web search engines.
...
You need a Baidu Qianfan AppBuilder API key to use this connector. The key is encrypted and stored securely by Corvos.
</Callout>
...
5. Copy the API key. Corvos uses it as the `BAIDU_API_KEY` connector setting.
...
4. Ask a current Chinese web query in chat, such as `今天中国人工智能行业有什么重要新闻？` (*"What important news is there in China's AI industry today?"*).

### Optional Advanced Settings

Corvos stores advanced Baidu options in the connector config. If your deployment exposes these fields, use the following values:
...
Corvos calls Baidu's intelligent search generation endpoint:
...
- Try a Chinese query with clear search intent, for example `百度智能云千帆 AppBuilder 最新功能` (*"Latest features of Baidu Intelligent Cloud Qianfan AppBuilder"*).
...
- If you self-host Corvos, verify that the backend container can reach `qianfan.baidubce.com`.
