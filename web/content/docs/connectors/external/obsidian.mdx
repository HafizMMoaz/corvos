---
title: Obsidian
description: Sync your Obsidian vault with the Corvos plugin
---

# Obsidian Plugin Setup Guide

Corvos integrates with Obsidian through the Corvos Obsidian plugin.

## How it works

The plugin runs inside your Obsidian app and pushes note updates to Corvos over HTTPS.
...
1. Open **Connectors** in Corvos and choose **Obsidian**.
2. Install the plugin (recommended via BRAT) using the steps below.
3. In Obsidian, open **Settings → Corvos**.  
4. Paste your Corvos API token from the user settings section.
5. Paste your Server URL in the plugin setting: either your Corvos main domain (if `/api/v1` rewrites are enabled) or your direct backend URL.
6. Choose the Workspace in the plugin, then the first sync should run automatically.
7. Confirm the connector appears as **Obsidian - &lt;vault&gt;** in Corvos.
...
3. Paste the repository: `https://github.com/HafizMMoaz/Corvos/`.
4. Select the latest plugin version, then click "Add plugin".
5. Open **Settings → Corvos** to finish setup.
...
1. Delete the old legacy Obsidian connector from Corvos.
2. Install and configure the Corvos Obsidian plugin using the quick start above.
...
- Regenerate and paste a fresh API token from Corvos.
...
- Avoid using localhost unless Corvos and Obsidian run on the same machine.
