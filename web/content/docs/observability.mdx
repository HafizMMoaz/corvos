---
title: Observability
description: Configure backend traces and metrics for Corvos
icon: Radar
---

Corvos uses OpenTelemetry for backend traces and metrics. Application logs
...
`backend/.env`. Add these values there:

```dotenv
CORVOS_ENABLE_OTEL=true
CORVOS_ENV=dev
OTEL_EXPORTER_OTLP_ENDPOINT=http://otel-lgtm:4317
OTEL_EXPORTER_OTLP_PROTOCOL=grpc
OTEL_RESOURCE_ATTRIBUTES=service.namespace=corvos
...
When deploying `backend/Dockerfile` directly instead of production
compose, use the same split: Corvos containers export to a collector, and
...
Corvos keeps project-specific spans behind `app.observability.otel`:
...
metrics. Corvos adds these project metrics from `app.observability.metrics`:
...
Corvos intentionally does not create an OpenTelemetry `LoggerProvider`,
...
id. Add these to `backend/.env` (local) or `docker/.env` (production);
