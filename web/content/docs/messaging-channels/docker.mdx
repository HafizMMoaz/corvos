...
`docker/.env` file. You do not need to edit `backend/.env.example`.
...
For localhost-only testing, the defaults are enough for the Corvos UI, but
public webhooks from Telegram, WhatsApp, and Slack require a public HTTPS
Corvos URL. Use your deployed domain or a tunnel such as Cloudflare Tunnel
or ngrok.
