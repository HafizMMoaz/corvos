...
The Slack messaging channel lets users mention the Corvos bot in Slack and
chat with the Corvos backend agent from a Slack thread.
...
1. Slack sends an `app_mention` event to Corvos.
2. Corvos verifies the Slack signature and stores the event in the gateway inbox.
3. Corvos resolves the Slack user binding to a Corvos user and workspace.
