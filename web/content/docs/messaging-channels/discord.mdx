...
The Discord messaging channel lets users mention the Corvos bot in Discord
and chat with the Corvos backend agent from a Discord channel.
...
- **Message Content Intent** so Corvos can read text after a bot mention.
...
2. Corvos stores the event in the durable gateway inbox.
3. Corvos resolves the Discord user binding to a Corvos user and workspace.
4. Corvos runs the backend agent with that user's permissions.
