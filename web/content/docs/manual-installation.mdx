...
description: Set up Corvos from source, component by component
icon: Wrench
---

This guide sets up Corvos without Docker (except for zero-cache, which is simplest to run as a container). Choose this path if you want to contribute to Corvos or need full control over each component. If you just want to run Corvos, the [Docker installation](/docs/docker-installation) is much faster.
...
**Authentication.** Corvos supports local email/password auth (the default, no setup needed) or Google OAuth login. For Google login you need an OAuth client from the [Google Cloud Console](https://console.cloud.google.com/apis/credentials) - the [Google connectors guide](/docs/connectors/external/google) walks through creating one.

**Document parsing (ETL).** Corvos converts uploaded files with one of three services:
...
Corvos uses [Rocicorp Zero](https://zero.rocicorp.dev/) for real-time updates (notifications, document status, chat comments, indexing progress). Zero replicates data from PostgreSQL via **logical replication**, which requires a one-time PostgreSQL configuration change.
...
```bash
psql -U postgres -d corvos -c "SHOW wal_level;"
...
```sql
ALTER USER corvos WITH REPLICATION;
GRANT CREATE ON DATABASE corvos TO corvos;
```
...
```bash
psql -U postgres -d corvos -c "SELECT pubname FROM pg_publication;"
...
```powershell
cd backend
uv run celery -A celery_worker.celery_app worker --loglevel=info --concurrency=1 --pool=solo --queues="corvos,corvos.connectors,corvos.gateway"
```
...
```bash
docker run -d --name corvos-zero-cache \
  -p 4848:4848 \
  --add-host=host.docker.internal:host-gateway \
  -e ZERO_UPSTREAM_DB="postgresql://postgres:postgres@host.docker.internal:5432/corvos?sslmode=disable" \
  -e ZERO_CVR_DB="postgresql://postgres:postgres@host.docker.internal:5432/corvos?sslmode=disable" \
  -e ZERO_CHANGE_DB="postgresql://postgres:postgres@host.docker.internal:5432/corvos?sslmode=disable" \
  -e ZERO_REPLICA_FILE=/data/zero.db \
  -e ZERO_ADMIN_PASSWORD=corvos-zero-admin \
...
docker logs -f corvos-zero-cache
```
...
The deps-only stack exposes zero-cache on port `4848`. Point the frontend at it by setting the zero-cache URL in `web/.env` (see the comments in `web/.env.example`).
...
The Corvos browser extension saves any webpage - including those behind authentication - straight into your knowledge base.
...
Load the built extension in your browser's developer mode and configure it with your Corvos API key. See the [Plasmo build docs](https://docs.plasmo.com/framework/workflows/build#with-a-specific-target) for details.
...
- **Real-time updates not working / stale UI**: Verify zero-cache is running (`curl http://localhost:4848/keepalive` returns 200). Open browser DevTools → Console and look for WebSocket errors. Confirm the zero-cache URL in `web/.env` matches the running zero-cache address.
- **Zero-cache stuck on `Unknown or invalid publications. Specified: [zero_publication]`**: You skipped `uv run alembic upgrade head`. Run it from `backend/`, then `docker restart corvos-zero-cache`.
- **Zero-cache crashes with `_zero.tableMetadata` errors**: A previous run left a half-built SQLite replica behind. Start fresh: `docker rm -f corvos-zero-cache && docker volume rm corvos-zero-cache`, then re-run the command from [Zero-Cache Setup](#zero-cache-setup).
