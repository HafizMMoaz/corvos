...
export interface RoutedSectionItem {
	...
	/** Renders the item as a non-interactive, greyed-out placeholder (e.g. an unbuilt tab). */
	disabled?: boolean;
}
...
function SectionNavLink({
	...
	if (item.disabled) {
		return (
			<span
				aria-disabled="true"
				className="inline-flex h-auto cursor-not-allowed items-center justify-start gap-3 rounded-md px-3 py-2.5 text-left text-sm font-medium text-muted-foreground/50"
			>
				{item.icon}
							...
							{items.map((item) =>
								item.disabled ? (
									<span
										key={item.value}
										aria-disabled="true"
										className="inline-flex h-auto shrink-0 cursor-not-allowed items-center gap-2 rounded-md px-3 py-1.5 text-xs font-medium text-muted-foreground/50"
									>
										{item.icon}
										{item.label}
									</span>
