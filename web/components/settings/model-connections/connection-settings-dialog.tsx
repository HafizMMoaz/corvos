...
export function ConnectionSettingsDialog({
	...
	async function handleAddManualModel(modelId: string) {
		const added = await addManualModel.mutateAsync({
			connectionId: connection.id,
			data: {
				model_id: modelId,
		...
		setDraftEnabledModelIds((current) => new Set(current).add(added.id));
	}
