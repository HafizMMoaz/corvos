...
export function ModelProviderConnectionsPanel({
	...
	addProviderDescription = "Corvos supports popular providers and self-hosted model endpoints.",
	...
	// Draft-only: the connection doesn't exist yet, so a manually-typed model
	// just joins the in-memory list and rides along with `handleCreate`'s
	// `models` payload. No API call here.
	function addManualConnectModel(modelId: string) {
		setConnectModels((current) => {
			if (current.some((item) => item.model_id === modelId)) {
				return current.map((item) =>
					item.model_id === modelId ? { ...item, enabled: true } : item
				);
			}
			return [
				...current,
	...
	return (
		<div className={className ?? "flex flex-col gap-6"}>
			...
			<ProviderConnectDialog
				...
				onAddManualPreviewModel={addManualConnectModel}
			/>
