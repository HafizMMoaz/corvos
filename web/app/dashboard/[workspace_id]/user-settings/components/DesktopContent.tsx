...
export function DesktopContent() {
		...
		return (
			<div className="flex flex-col items-center justify-center py-12 text-center">
				<p className="text-sm text-muted-foreground">
					App preferences are only available in the Corvos desktop app.
				</p>
			</div>
		);
	}
	...
	const handleAutoLaunchToggle = async (checked: boolean) => {
		...
		try {
			...
			toast.success(checked ? "Corvos will launch on startup" : "Launch on startup disabled");
		} catch {
			setAutoLaunchEnabled(!checked);
		...
		<div className="flex flex-col gap-4 md:gap-6">
			...
			<section>
				<div className="pb-2 md:pb-3">
					...
					<p className="text-xs md:text-sm text-muted-foreground">
						Automatically start Corvos when you sign in to your computer so global shortcuts and
				...
				<div className="flex flex-col gap-3">
					<div className="flex items-center justify-between rounded-lg bg-accent p-4">
						<div className="space-y-0.5">
							<Label htmlFor="auto-launch-toggle" className="text-sm font-medium cursor-pointer">
								Open Corvos at login
							</Label>
							<p className="text-xs text-muted-foreground">
								{autoLaunchSupported
									? "Adds Corvos to your system's login items."
					...
					<div className="flex items-center justify-between rounded-lg bg-accent p-4">
						<div className="space-y-0.5">
							...
							<p className="text-xs text-muted-foreground">
								Skip the main window on boot. Corvos lives in the system tray until you need it.
							</p>
						</div>
						<Switch
