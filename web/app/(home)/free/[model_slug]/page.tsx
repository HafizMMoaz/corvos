...
function buildSeoTitle(model: AnonModel): string {
	...
	return `Chat with ${model.name} Free, No Login | Corvos`;
}

function buildSeoDescription(model: AnonModel): string {
	if (model.seo_description) return model.seo_description;
	return `Use ${model.name} free online without login. No sign-up required. Chat with ${model.name} by ${model.provider} instantly on Corvos, the open source ChatGPT alternative with no login.`;
}
	...
	return [
		{
			...
			answer: `Yes. ${model.name} is available on Corvos without login. No account creation, no email, no password. Just open the page and start chatting with ${model.name} for free.`,
		},
		{
			question: `Is ${model.name} really free on Corvos?`,
			answer: `Yes! You can use ${model.name} completely free without login or sign-up. Corvos gives you 500,000 free tokens to use across any model, including ${model.name}.`,
		},
		{
		...
		{
			question: `What can I do with ${model.name} on Corvos?`,
			answer: `You can ask questions, get explanations, write content, brainstorm ideas, debug code, and more. ${model.name} is a powerful AI assistant available for free without login on Corvos.`,
		},
		{
			question: `How is Corvos different from using ${model.name} directly?`,
			answer: `Corvos gives you free access without login to ${model.name} and many other AI models in one place. Create a free account to unlock document Q&A, team collaboration, and integrations with Slack, Google Drive, Notion, and more.`,
		},
	];
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
	...
	if (!model) return { title: "Model Not Found | Corvos" };

	const title = buildSeoTitle(model);
	const description = buildSeoDescription(model);
	const canonicalUrl = `https://www.corvos.com/free/${model.seo_slug}`;
			...
			images: [
				{
					...
					width: 1200,
					height: 630,
					alt: `${model.name} Free Chat on Corvos`,
				},
			],
		},
		twitter: {
		...
		<>
			...
			<JsonLd
				data={{
					...
					"@type": "WebApplication",
					name: `${model.name} Free Chat Without Login - Corvos`,
					description,
					url: `https://www.corvos.com/free/${model.seo_slug}`,
				...
				<article className="container mx-auto px-4 py-10 max-w-3xl">
					<header className="mb-6">
						...
						<p className="text-sm text-muted-foreground leading-relaxed">
							...
							email, no password needed. Powered by Corvos.
						</p>
					</header>
