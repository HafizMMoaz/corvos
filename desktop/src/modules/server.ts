...
function getStandalonePath(): string {
  if (isDev) {
    return path.join(__dirname, '..', '..', 'web', '.next', 'standalone', 'web');
  }
...
export async function startNextServer(): Promise<void> {
  ...
  const backendInternalUrl = process.env.CORVOS_BACKEND_INTERNAL_URL || process.env.HOSTED_BACKEND_URL;

  const child = utilityProcess.fork(serverScript, [], {
    cwd: standalonePath,
    env: {
      ...process.env,
      ...(backendInternalUrl ? { CORVOS_BACKEND_INTERNAL_URL: backendInternalUrl } : {}),
      PORT: String(serverPort),
      // Loopback bind: avoids 0.0.0.0 leaking into request.url and redirect origins.
      HOSTNAME: SERVER_HOST,
      NODE_ENV: 'production',
    },
    serviceName: 'Corvos Next Server',
    stdio: 'pipe',
  });
