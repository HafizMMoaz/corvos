    ...
    const server = http.createServer(async (req, res) => {
      try {
        ...
        if (url.pathname !== OAUTH_CALLBACK_PATH) {
          writeOAuthPage(res, 404, 'Not found', 'This OAuth callback endpoint is only used by Corvos.');
          return;
        }
        ...
        if (oauthError) {
          ...
          writeOAuthPage(res, 400, 'Authentication failed', 'You can close this window and return to Corvos.', 'error');
        ...
        if (!port) {
          writeOAuthPage(res, 500, 'Authentication failed', 'You can close this window and return to Corvos.', 'error');
        ...
        if (!response.ok) {
          ...
          writeOAuthPage(res, 401, 'Authentication failed', 'You can close this window and return to Corvos.', 'error');
          fail(new Error(detail));
          return;
        }
        const tokens = (await response.json()) as DesktopAuthTokens;
        writeOAuthPage(res, 200, 'Authentication complete', 'You can close this window and return to Corvos.', 'success');
        succeed(tokens);
      } catch (error) {
