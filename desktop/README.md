...
- Node.js 18+
- pnpm 10+
- The `web` project dependencies installed (`pnpm install` in `web/`)
...
On **Linux**, `pnpm dev` runs Electron through `scripts/electron-dev.mjs`: it sets `ELECTRON_DISABLE_SANDBOX=1` for the sandbox issue and passes **`--ozone-platform=x11`** (XWayland) unless **`CORVOS_ELECTRON_WAYLAND=1`** is set, so dev tends to behave closer to X11 for shortcuts and Ozone. Packaged Linux builds are unchanged.

## Configuration

Two `.env` files control the build:

**`web/.env`** - Next.js environment variables baked into the frontend at build time:

**`desktop/.env`** - Electron-specific configuration:
