...
export class StatusBar {
	...
	constructor(host: HTMLElement, onClick?: () => void) {
		this.el = host;
		this.el.addClass("corvos-status");
		this.icon = this.el.createSpan({ cls: "corvos-status__icon" });
		this.text = this.el.createSpan({ cls: "corvos-status__text" });
		if (onClick) {
			this.el.addClass("corvos-status--clickable");
	...
	update(state: StatusState): void {
		...
		this.el.removeClass("corvos-status--err");
		if (visual.isError) this.el.addClass("corvos-status--err");
		setIcon(this.icon, visual.icon);

		let label = `Corvos: ${visual.label}`;
