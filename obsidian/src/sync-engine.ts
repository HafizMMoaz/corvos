...
export class SyncEngine {
	...
	async start(): Promise<void> {
		this.setStatus("syncing", "Connecting to Corvos…");
		...
		if (upserts.length > 0) {
			...
			for (const item of upserts) {
				...
				try {
					...
					console.error("Corvos: failed to build payload", item.path, err);
					retry.push(item);
				}
			}
	...
	private async handleVaultNotRegistered(err: unknown): Promise<boolean> {
		...
		console.warn("Corvos: vault not registered, re-connecting before retry", err);
		await this.ensureConnected();
		return true;
	}
	...
	private classify(err: unknown): "ack" | "retry" | "drop" | "stop" {
		...
		if (err instanceof PermanentError) {
			console.warn("Corvos: permanent error, dropping batch", err);
			new Notice(`Corvos: ${err.message}`);
			return "drop";
		}
		console.error("Corvos: unknown error", err);
		return "retry";
	}
