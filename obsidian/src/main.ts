...
import {
	...
	type QueueItem,
	type StatusState,
	type CorvosPluginSettings,
} from "./types";
import { generateVaultUuid } from "./vault-identity";

/** Corvos plugin entry point. */
export default class CorvosPlugin extends Plugin {
	settings!: CorvosPluginSettings;
	api!: CorvosApiClient;
	...
	private settingTab: CorvosSettingTab | null = null;
	...
	async onload() {
		...
		this.settingTab = new CorvosSettingTab(this.app, this);
		...
		this.addCommand({
			...
			callback: async () => {
				try {
					...
					new Notice("Corvos: re-sync started.");
				} catch (err) {
					new Notice(`Corvos: re-sync failed - ${(err as Error).message}`);
				}
			},
		});
	...
	private notifyAuthError(): void {
		...
		new Notice("Corvos: API token is invalid or expired. Check your token in settings.", 8000);
	}
	...
	async loadSettings() {
		...
		const data = (await this.loadData()) as
			| (Partial<CorvosPluginSettings> & { searchSpaceId?: number | null })
			| null;
