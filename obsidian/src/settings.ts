...
export class CorvosSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName("Server URL")
			.setDesc(
				"https://corvos.com for Corvos Cloud, or your self-hosted URL.",
			)
			.addText((text) =>
				text
					.setPlaceholder("https://corvos.com")
		...
		new Setting(containerEl)
			.setName("API token")
			.setDesc(
				"Paste your Corvos personal access token from the web app.",
			)
			.addText((text) => {
			...
			.addButton((btn) => {
				...
				btn.setButtonText("Verify").setCta().onClick(async () => {
					if (this.plugin.settings.apiToken.trim().length === 0) {
						new Notice("Corvos: paste an API token before verifying.");
						return;
					}
					btn.setDisabled(true);
					try {
						await this.plugin.api.verifyToken();
						new Notice("Corvos: token verified.");
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Which Corvos workspace this vault syncs into. Reload after changing your token.",
			)
			.addDropdown((drop) => {
				...
				drop.onChange(async (value) => {
					...
					if (this.plugin.settings.workspaceId !== null) {
						try {
							...
							new Notice("Corvos: vault connected.");
							this.display();
						} catch (err) {
							this.handleApiError(err);
						}
		...
		new Setting(containerEl)
			...
			.addButton((btn) =>
				btn.setButtonText("Open releases").onClick(() => {
					window.open(
						"https://github.com/HafizMMoaz/Corvos/releases?q=obsidian",
						"_blank",
					);
				}),
			);
	...
	private renderConnectionHeading(containerEl: HTMLElement): void {
		...
		heading.nameEl.addClass("corvos-connection-heading");
		this.connectionIndicator = heading.nameEl.createSpan({
			cls: "corvos-connection-indicator",
		});
	...
	private updateConnectionIndicator(): void {
		...
		indicator.empty();
		indicator.removeClass("corvos-connection-indicator--err");
		if (visual.isError) {
			indicator.addClass("corvos-connection-indicator--err");
		}
	...
	private handleApiError(err: unknown): void {
		...
		new Notice(
			`Corvos: request failed - ${(err as Error).message ?? "unknown error"}`,
		);
	}
}
