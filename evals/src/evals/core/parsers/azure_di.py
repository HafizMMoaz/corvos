...
``backend/app/etl_pipeline/parsers/azure_doc_intelligence.py``
...
async def parse_with_azure_di(
    ...
    *,
    ...
    ``AZURE_DI_KEY`` env vars (set in ``evals/.env``).
    ...
    if not endpoint or not api_key:
        raise ValueError(
            "AZURE_DI_ENDPOINT and AZURE_DI_KEY must be set (see evals/.env)."
        )

    model_id = _AZURE_MODEL_BY_MODE.get(processing_mode, "prebuilt-read")

    # Lazy imports - evals shouldn't pay the azure-sdk
