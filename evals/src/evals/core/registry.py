...
class RunContext:
    ...
    ``http`` is the authenticated Corvos client (auth event hook
    ...
    def provider_model(self) -> str:
        """Slug used by the Corvos agent (and the native arm by default).

        For ``cost-arbitrage`` scenarios this is the *cheap, text-only*
        slug - Corvos answers from the chunks the vision LLM already
    ...
    def vision_provider_model(self) -> str | None:
        """Slug of the OpenRouter vision LLM Corvos used at ingest.
    ...
    def search_space_client(self) -> SearchSpaceClient:
        return SearchSpaceClient(self.http, self.config.corvos_api_base)

    def documents_client(self) -> DocumentsClient:
        return DocumentsClient(self.http, self.config.corvos_api_base)

    def new_chat_client(self) -> NewChatClient:
        return NewChatClient(self.http, self.config.corvos_api_base)
