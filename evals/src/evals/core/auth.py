...
* **GOOGLE / pre-issued JWT**: operator pastes their existing JWT (and
  optionally refresh token) into ``CORVOS_JWT`` /
  ``CORVOS_REFRESH_TOKEN``; harness skips login.
...
_NO_CREDENTIALS_MESSAGE = (
    "No Corvos credentials configured. Set ONE of:\n"
    "  (LOCAL)  CORVOS_USER_EMAIL + CORVOS_USER_PASSWORD\n"
    "  (GOOGLE) CORVOS_JWT (and optionally CORVOS_REFRESH_TOKEN)\n"
    "For GOOGLE: use a PAT or operator-issued bearer token and set "
    "CORVOS_JWT (plus CORVOS_REFRESH_TOKEN if available)."
)
...
async def acquire_token(config: Config, *, http: httpx.AsyncClient | None = None) -> TokenBundle:
    ...
    1. ``CORVOS_JWT`` set → use it directly. Refresh token captured if
       supplied.
    2. ``CORVOS_USER_EMAIL`` + ``CORVOS_USER_PASSWORD`` set →
    ...
    if config.has_jwt_mode():
        return TokenBundle(
            access_token=config.corvos_jwt or "",
            refresh_token=config.corvos_refresh_token,
            mode="jwt",
        )
    ...
    if config.has_local_mode():
        ...
        async def _login(client: httpx.AsyncClient) -> TokenBundle:
            response = await client.post(
                f"{config.corvos_api_base}/auth/desktop/login",
                json={
                    "email": config.corvos_user_email,
                    "password": config.corvos_user_password,
                },
    ...
    try:
        async with httpx.AsyncClient(
            ...
            response = await inner.post(
                f"{state.config.corvos_api_base}/auth/jwt/refresh",
...
def client_with_auth(
    ...
    *,
    ...
    """Build a single shared ``httpx.AsyncClient`` for the Corvos API.
