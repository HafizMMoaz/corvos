...
def _discover_suites() -> list[str]:
    ...
    from evals.suites import discover_suites
...
def _resolve_openrouter_id(
    ...
    *,
    ...
    """Resolve the Corvos LLM id for ``provider_model``.
    ...
    * Otherwise: filter to ``provider == "OPENROUTER"`` and
    ...
    if not matches:
        ...
        raise RuntimeError(
            ...
            "Browse with: python -m evals models list --grep <substring>"
        )
    if len(matches) > 1:
...
async def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    if scenario == "cost-arbitrage":
        if not native_arm_model:
            console.print(
                ...
                "model to fairly answer image-bearing questions; Corvos "
    ...
    async with client_with_auth(config, token) as http:
        candidates = await _list_global_llm_configs(http, config.corvos_api_base)
        ...
        ss_client = SearchSpaceClient(http, config.corvos_api_base)
        ...
        if existing is None:
            ...
            row = await ss_client.create(
                ss_name, description=f"corvos-evals lifecycle ({suite})"
            )
            console.print(
        ...
        # requirement - without it, Corvos's chunks have no image
...
async def _cmd_models_list(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 2
    async with client_with_auth(config, token) as http:
        entries = await _list_global_llm_configs(http, config.corvos_api_base)
...
def _cmd_suites_list(_args: argparse.Namespace) -> int:
    ...
    if not suites:
        console.print(
            ...
            "src/evals/suites/<domain>/<benchmark>/.[/yellow]"
        )
        return 0
...
async def _cmd_ingest(args: argparse.Namespace) -> int:
    ...
    if state is None:
        console.print(
            ...
            f"`python -m evals setup --suite {args.suite} "
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="corvos-evals",
        description="Corvos evaluation harness - domain-agnostic core + pluggable suites.",
    )
    parser.add_argument(
    ...
    p_setup.add_argument(
        ...
        required=True,
        help=(
            "OpenRouter slug for the Corvos answer LLM (and the native arm "
    ...
    p_setup.add_argument(
        ...
        help=(
            ...
            "Corvos pre-extracted images at ingest with a vision LLM; "
            "cost-arbitrage: native arm uses --native-arm-model (vision), "
            "Corvos uses --provider-model (cheap, text-only) over chunks "
    ...
    p_setup.add_argument(
        ...
        metavar="SLUG",
        help=(
            "OpenRouter slug for the vision LLM Corvos uses at ingest "
    ...
    p_setup.add_argument(
        ...
        help=(
            ...
            "by the native_pdf arm only; Corvos answers with "
