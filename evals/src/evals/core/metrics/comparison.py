...
and Corvos). That makes per-question outcomes paired, so
...
class McnemarResult:
    ...
    n_total: int
    b: int  # native correct, corvos wrong
    c: int  # native wrong,   corvos correct
    statistic: float
    p_value: float
    method: str
...
def mcnemar_test(
    ...
    *,
    ...
    "corvos"). The test statistic only depends on discordant pairs.
