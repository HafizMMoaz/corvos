...
``~5 MB`` markdown bundles for Corvos's file upload (smaller
...
* ``mirage_chunk_map.jsonl`` - ``{chunk_id, document_id, snippet_id?}``
  ...
  snippet it overlaps when the Corvos chunker splits a batched
  markdown).
"""
...
async def run_ingest(
    ...
    *,
    ...
    chosen corpus and let Corvos's own retriever do the work. The
    ...
    # Best-effort chunk map. Corvos doesn't expose snippet attribution
