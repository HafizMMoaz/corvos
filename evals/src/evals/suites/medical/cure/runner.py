...
Corvos's standard ``[citation:CHUNK_ID]`` format (already required
...
from ....core.arms import ArmRequest, ArmResult, CorvosArm
...
_DESCRIPTION = "CUREv1 retrieval (single-arm Corvos): Recall@k / MRR / nDCG@10."
    ...
    async def run(self, ctx: RunContext, **opts: Any) -> RunArtifact:
        ...
        if not chunk_to_doc or not doc_to_corpus:
            raise RuntimeError(
                ...
                "`python -m evals ingest medical cure` first."
            )
    ...
    def report_section(self, artifacts: list[RunArtifact]) -> ReportSection:
        if not artifacts:
            return ReportSection(
                title="CUREv1 - single-arm Corvos retrieval",
                headline=False,
