...
* ``build_corvos_prompt(q)``    - question + a hint that retrieval
  over the question's 5 ingested pages is available; the Corvos
...
def build_corvos_prompt(question: str, *, query_time: str = "") -> str:
    """Prompt for the Corvos arm (agent does retrieval itself)."""

    return _CORVOS_TEMPLATE.format(
        instructions=_BASE_INSTRUCTIONS,
        question=question.strip(),
        query_time=query_time.strip() or "unknown",
    )
