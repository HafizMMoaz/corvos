...
3. ``corvos``     - POST ``/api/v1/new_chat`` with retrieval scoped
...
flag for each arm pair (long_context vs bare, corvos vs
long_context, corvos vs bare) so the report can call out exactly
where the lift comes from. Per-domain and per-question-type breakdowns
surface where Corvos beats long-context (e.g. multi-hop / set
...
from ....core.arms import ArmRequest, ArmResult, BareLlmArm, CorvosArm
...
_DESCRIPTION = (
    ...
    "arms (Bare LLM / Long-Context LLM / Corvos) over the same "
...
class CragBenchmark:
    """3-arm CRAG runner: bare vs long-context vs Corvos."""
    ...
    def add_run_args(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            dest="skip_corvos",
            action="store_true",
            help="Skip the Corvos arm (useful when iterating on the LLM arms only).",
        )
        parser.add_argument(
            "--no-mention-scope",
            ...
            action="store_true",
            help=(
                "Corvos arm: don't pass mentioned_document_ids; let "
        ...
        parser.add_argument(
            ...
            dest="skip_upload",
            action="store_true",
            help="(ingest only) extract pages locally but don't push to Corvos.",
        )
        parser.add_argument(
            "--overwrite-extract",
    ...
    async def run(self, ctx: RunContext, **opts: Any) -> RunArtifact:
        ...
        skip_corvos = bool(opts.get("skip_corvos"))
        ...
        if not map_path.exists():
            raise RuntimeError(
                ...
                "`python -m evals ingest research crag --n-questions 200` first."
            )
        ...
        async def _surf_one(q: CragRunnerQuestion) -> ArmResult:
            ...
            return await surf_arm.answer(
                _make_corvos_request(q, scope_to_pages=not no_mention_scope)
            )
        ...
        if surf_arm is not None:
            ...
            tasks.append(_make_skipped_results(questions, "corvos"))
    ...
    def report_section(self, artifacts: list[RunArtifact]) -> ReportSection:
        if not artifacts:
            return ReportSection(
                title="CRAG - Bare LLM vs Long-Context LLM vs Corvos",
                headline=True,
        ...
        if not active.get("corvos", True):
            body_lines.append("- Corvos arm: SKIPPED.")
        else:
            body_lines.append(
                f"- Corvos arm (`{extra.get('provider_model', '?')}`, retrieval over "
        ...
        if deltas:
            ...
            for label, key in (
                ("Corvos vs Bare", "corvos_vs_bare"),
                ("Corvos vs Long-Context", "corvos_vs_long_context"),
        ...
        if per_domain:
            ...
            for domain in sorted(per_domain.keys()):
                ...
                for arm in ("bare_llm", "long_context", "corvos"):
                    if arm not in row:
                        continue
...
def _make_corvos_request(q: CragRunnerQuestion, *, scope_to_pages: bool) -> ArmRequest:
    mentions: list[int] | None = None
    if scope_to_pages and q.document_ids:
        mentions = list(q.document_ids)
    return ArmRequest(
        question_id=q.qid,
        prompt=build_corvos_prompt(q.question, query_time=q.query_time),
...
def _compute_metrics(
    *,
    ...
    out: dict[str, Any] = {
        ...
        "corvos": _arm_block(surf_results, surf_acc, surf_t),
    }
    ...
    for label, ref_correct, ref_t, chal_correct, chal_t, both_active in (
        (
            ...
            surf_correct,
            surf_t,
            arms_active.get("bare_llm") and arms_active.get("corvos"),
        ),
        (
            "corvos_vs_long_context",
            lc_correct,
            lc_t,
            surf_correct,
            surf_t,
            arms_active.get("long_context") and arms_active.get("corvos"),
        ),
        (
    ...
    out["grader_methods"] = {
        ...
        "corvos": _count_methods(surf_grades) if arms_active.get("corvos") else {},
    }
    return out
...
def _per_facet_truthfulness(
    ...
    *,
    ...
    for q, b, lc, s in zip(questions, bare_grades, lc_grades, surf_grades, strict=False):
        key = key_fn(q)
        bucket = buckets.setdefault(key, {"bare_llm": [], "long_context": [], "corvos": []})
        bucket["bare_llm"].append(b)
        bucket["long_context"].append(lc)
        bucket["corvos"].append(s)
    out: dict[str, Any] = {}
