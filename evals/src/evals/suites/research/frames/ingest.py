...
6. Upload the resulting markdown files to Corvos in batches with
...
We resolve titles → Corvos document_ids via the post-upload
``DocumentStatus.title`` field. Corvos's title is the uploaded
...
async def _upload_markdowns(
    ...
    *,
    ...
    Corvos dedupes uploads on ``(filename, search_space_id)``, so
    ...
    for batch_start in range(0, len(paths), batch_size):
        ...
        if all_ids:
            ...
            for s in statuses:
                # Corvos stores the uploaded filename as ``title`` (no extension).
...
async def run_ingest(
    ...
    *,
    ...
    skip_upload : bool
        ...
        Corvos. Useful for debugging the fetcher in isolation.
    ...
    # 4. Upload to Corvos (deduped by filename).
    name_to_id: dict[str, int] = {}
    if skip_upload:
        logger.info("FRAMES: --skip-upload; skipping Corvos ingestion")
    else:
