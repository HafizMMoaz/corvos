...
* ``document_id``    - Corvos document id (carried over from
                        mmlongbench's existing ingestion so the
                        Corvos agentic arm can scope retrieval).
...
arm, and uses ``document_id`` for the Corvos arm.
...
parser quality on the same physical PDFs Corvos already has, so
...
async def run_ingest(
    ...
    *,
    ...
    # ingestion. parser_compare doesn't re-upload; Corvos's agentic
