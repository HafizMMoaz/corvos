...
* ``corvos_agentic``    - Corvos /api/v1/new_chat retrieval over chunks.

Cost reporting:

* ``llm_cost_per_q``       - mean OpenRouter ``usage.cost`` reported by
                              the chat-completions API. Zero for the
                              Corvos agentic arm because the SSE
...
# The Corvos agentic arm is fed by the existing mmlongbench
...
_DESCRIPTION = (
    ...
    "stuffing + Corvos agentic retrieval. Reports preprocessing dollars "
...
class ParserCompareBenchmark:
    ...
    def add_run_args(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            dest="no_mentions",
            action="store_true",
            help="Corvos arm: skip mentioned_document_ids (full-corpus retrieval).",
        )
        parser.add_argument(
            "--pdf-engine",
            default="native",
        ...
        parser.add_argument(
            ...
            "Corvos arm uses whatever provider_model is pinned on the suite.",
        )
        parser.add_argument(
            "--skip-arms",
            default=None,
    ...
    async def run(self, ctx: RunContext, **opts: Any) -> RunArtifact:
        ...
        if not map_path.exists():
            raise RuntimeError(
                ...
                "`python -m evals ingest multimodal_doc parser_compare` first."
            )
        ...
        if "corvos_agentic" in active_arms:
            surf = CorvosArm(
                client=ctx.new_chat_client(),
                search_space_id=ctx.search_space_id,
                ephemeral_threads=True,
            )
            # Override the default "corvos" name so the metrics
            # bucket lines up with the rest of parser_compare's arms.
            surf.name = "corvos_agentic"
            arms["corvos_agentic"] = surf
        ...
        def _surf_req(q: PCQuestion) -> ArmRequest:
            ...
            return ArmRequest(
                question_id=q.qid,
                prompt=build_corvos_prompt(q.question, answer_format=q.answer_format),
    ...
    def report_section(self, artifacts: list[RunArtifact]) -> ReportSection:
        ...
        body.append("")
        body.append(
            "- `corvos_agentic` LLM cost shows as $0.0000 because the "
...
def _compute_metrics(
    ...
    for arm_name in active_arms:
        ...
        elif arm_name == "corvos_agentic":
            preprocess_per_page = PREPROCESS_USD_PER_PAGE[CORVOS_INGEST_MODE]
            preprocess_label = (
                f"{CORVOS_INGEST_MODE} tier (ingested by Corvos at "
