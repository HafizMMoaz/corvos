...
* ``build_corvos_prompt``        - bare question + format hint
...
def build_corvos_prompt(question: str, *, answer_format: str) -> str:
    """Prompt for ``CorvosArm`` - chunks retrieved by the agent."""

    # Corvos's agent already injects retrieved chunks via its tool
    # loop; the prompt only carries the user-visible question + format
    # hint, mirroring how a human asks the Corvos UI.
    return f"{_BASE_INSTRUCTION}\n\nQuestion: {question.strip()}\n\n{_format_hint(answer_format)}\n"
