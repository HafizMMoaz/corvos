...
from evals.core.auth import (
    CredentialError,
    acquire_token,
    client_with_auth,
)
from evals.core.config import Config


def _make_config(**overrides) -> Config:
    base = {
        "corvos_api_base": "http://test",
        "openrouter_api_key": None,
        "openrouter_base_url": "https://openrouter.ai/api/v1",
        "corvos_jwt": None,
        "corvos_refresh_token": None,
        "corvos_user_email": None,
        "corvos_user_password": None,
        "data_dir": None,
        "reports_dir": None,
    }
...
async def test_acquire_token_jwt_mode_short_circuits():
    config = _make_config(corvos_jwt="abc", corvos_refresh_token="ref")
...
async def test_acquire_token_local_mode_posts_desktop_login_json():
    ...
    config = _make_config(corvos_user_email="u@example.com", corvos_user_password="pw")
    bundle = await acquire_token(config)
...
async def test_acquire_token_no_credentials():
    ...
    assert "CORVOS_USER_EMAIL" in str(exc.value)
    assert "CORVOS_JWT" in str(exc.value)


@pytest.mark.asyncio
@respx.mock
async def test_client_with_auth_refreshes_on_401():
    config = _make_config(corvos_jwt="old", corvos_refresh_token="ref")
    bundle = await acquire_token(config)
