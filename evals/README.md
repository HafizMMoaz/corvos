...
|---------------------------------|--------------------------------------------------|------------------|----------------------------|
| `medical/medxpertqa` (headline) | Native PDF vs Corvos head-to-head, MCQ        | yes              | `vision=on, mode=basic`    |
| `medical/mirage`                | Corvos single-arm, MCQ                        | no               | `vision=off, mode=basic`   |
| `medical/cure`                  | Corvos single-arm retrieval (Recall/MRR/nDCG) | no               | `vision=off, mode=basic`   |
| `multimodal_doc/mmlongbench`    | Native PDF vs Corvos head-to-head, open-ended | yes              | `vision=on, mode=basic`    |
...
# Edit .env: CORVOS_API_BASE, OPENROUTER_API_KEY, and ONE of:
#   LOCAL  → CORVOS_USER_EMAIL + CORVOS_USER_PASSWORD
#   GOOGLE → CORVOS_JWT (+ optional CORVOS_REFRESH_TOKEN)
...
python -m evals suites list
python -m evals benchmarks list

# 1. MEDICAL SUITE - one SearchSpace, three benchmarks
python -m evals setup --suite medical --provider-model anthropic/claude-sonnet-4.5

#  1a. headline head-to-head: Native PDF (vision) vs Corvos (vision RAG)
...
python -m evals ingest medical medxpertqa --split test
python -m evals run    medical medxpertqa --concurrency 4

#  1b. MIRAGE - single-arm Corvos MCQ accuracy
#      (MMLU-Med / MedQA-US / MedMCQA / PubMedQA / BioASQ)
python -m evals ingest medical mirage
python -m evals run    medical mirage

#  1c. CUREv1 - single-arm Corvos retrieval (Recall@k / MRR / nDCG@10)
python -m evals ingest medical cure --lang en
python -m evals run    medical cure --lang en

#  1d. write reports/medical/<UTC-ts>/summary.{md,json}
python -m evals report --suite medical

# 2. MULTIMODAL_DOC SUITE - long PDFs with embedded images, charts, tables
python -m evals setup  --suite multimodal_doc --provider-model google/gemini-2.5-pro
python -m evals ingest multimodal_doc mmlongbench           # ~660MB, resumable
python -m evals run    multimodal_doc mmlongbench --concurrency 4
python -m evals report --suite multimodal_doc

# 3. CLEANUP - soft-deletes the SearchSpaces; rendered PDFs stay cached
python -m evals teardown --suite medical
python -m evals teardown --suite multimodal_doc
```

## Asymmetric scenarios - the "vision-extract once, answer cheap" play

The walkthrough above is `--scenario head-to-head` (default): both arms answer with the same vision-capable slug. Corvos's actual architectural value-prop is that the **ingestion-time vision LLM and the runtime LLM are completely independent** - you can pay a vision LLM *once*, at ingest, to convert every embedded image into text (per-image OCR **and** semantic description, inlined where the image actually appears in the document - see [What `--use-vision-llm` produces](#what---use-vision-llm-produces) below). Then every query is served by a cheap text-only model that sees that extracted text natively. Two extra scenarios make this explicit:

| `--scenario`       | Native arm answers with                | Corvos arm answers with     | Question being measured                                                                  |
|--------------------|----------------------------------------|--------------------------------|------------------------------------------------------------------------------------------|
...
| `cost-arbitrage`   | `--native-arm-model` (vision)          | `--provider-model` (cheap)     | How close does Corvos get to a vision-native baseline at a fraction of per-query cost?|
...
This is the answer to *"does Corvos give a non-vision LLM enough context to reason over image-heavy docs?"*. Both arms hit the same cheap text-only slug. The native arm is structurally blind to images (text-only LLM + raw PDFs). The Corvos arm reads chunks that already contain the per-image OCR and visual descriptions, written there by the vision LLM at ingest time.

```bash
python -m evals setup --suite medical \
...
python -m evals ingest medical medxpertqa --split test  # vision=on by default
python -m evals run    medical medxpertqa --concurrency 4
python -m evals report --suite medical
...
### Cheap Corvos vs vision-native baseline (`cost-arbitrage`)
...
python -m evals setup --suite medical \
  ...
  # native arm = sonnet (vision); Corvos arm = gpt-5.4-mini (text-only)
...
#   (vision); Corvos answers with `openai/gpt-5.4-mini` over chunks vision-extracted
#   at ingest by `anthropic/claude-sonnet-4.5`.
```

Notes:
- `cost-arbitrage` requires both `--provider-model` (the cheap Corvos slug) AND `--native-arm-model <vision slug>`.
...
- `--no-mentions` - let Corvos retrieve unscoped ("did the @-mention matter?")
...
This is what makes `--scenario symmetric-cheap` and `--scenario cost-arbitrage` work: a non-vision LLM reading Corvos's chunks sees the image's text and semantic content as plain markdown, alongside the surrounding case text, in the same retrieved chunk. Without it the cheap LLM would have nothing extra to read.

### A/B testing the same corpus with different settings

Corvos dedupes uploads by `(filename, search_space_id)` - **not** by content hash and **not** by ingestion settings. Re-uploading the same filename to the same SearchSpace with a different `--use-vision-llm` flag silently skips re-processing. Give each variant its own SearchSpace:

```bash
# Baseline arm (vision off)
python -m evals setup    --suite medical --provider-model anthropic/claude-sonnet-4.5
python -m evals ingest   medical medxpertqa --no-vision-llm
python -m evals run      medical medxpertqa --n 100
python -m evals teardown --suite medical

# Vision arm (the benchmark default)
python -m evals setup    --suite medical --provider-model anthropic/claude-sonnet-4.5
python -m evals ingest   medical medxpertqa
python -m evals run      medical medxpertqa --n 100
python -m evals report   --suite medical
```
...
- `CORVOS_API_BASE` (default `http://localhost:8000`)
- `OPENROUTER_API_KEY` - required for the `native_pdf` arm and for `models list`
- One of `CORVOS_USER_EMAIL` + `CORVOS_USER_PASSWORD` (LOCAL), **or** `CORVOS_JWT` (+ optional `CORVOS_REFRESH_TOKEN`) for GOOGLE/pre-issued JWT
...
1. Create `evals/src/evals/suites/<domain>/<benchmark>/` with `__init__.py`, `ingest.py`, `runner.py`, optional `prompt.py`.
...
- A long-context-stuffing arm (give the model the same retrieved chunks Corvos saw).
