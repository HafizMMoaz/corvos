...
* **No Corvos backend or celery required.** Corvos had 0
  reported failures; this script will skip any ``corvos_agentic``
...
from evals.core.arms import (  # noqa: E402
    ArmRequest,
    ArmResult,
    BareLlmArm,
    NativePdfArm,
)
from evals.core.parse.freeform_answer import (  # noqa: E402
    extract_freeform_answer,
)
from evals.core.providers.openrouter_chat import (  # noqa: E402
    OpenRouterChatProvider,
)
from evals.core.providers.openrouter_pdf import (  # noqa: E402
    OpenRouterPdfProvider,
    PdfEngine,
)
from evals.suites.multimodal_doc.mmlongbench.grader import grade  # noqa: E402
from evals.suites.multimodal_doc.parser_compare.prompt import (  # noqa: E402
...
async def _run(args: argparse.Namespace) -> int:
    ...
    # Corvos rows: warn and skip; we don't want to start backend just to
    # defensively retry a 0-failure arm.
    surf_failed = [f for f in failed if f.arm == "corvos_agentic"]
    if surf_failed:
        logger.warning(
            "Skipping %d corvos_agentic failures; this script doesn't drive the backend. "
            "If you want those retried too, start backend + celery and rerun "
            "with --include-corvos.",
            len(surf_failed),
        )
        if not args.include_corvos:
            failed = [f for f in failed if f.arm != "corvos_agentic"]
    else:
        logger.info("No corvos_agentic failures; backend/celery not needed for this retry.")
...
def main() -> None:
    ...
    parser.add_argument(
        "--include-corvos",
        action="store_true",
        help="Also retry corvos_agentic failures (requires backend + celery up). "
        "Default is to skip them since the n=171 run had 0 Corvos failures.",
    )
