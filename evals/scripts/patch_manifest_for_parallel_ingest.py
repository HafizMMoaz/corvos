...
Corvos ``document_id`` (the runner does, later, after a refresh).
...
Corvos ``document_id``s yet) so parser_compare can run all four
extractions for them in parallel with the Corvos ingest.

After mmlongbench finishes, re-run::

    python -m evals ingest multimodal_doc parser_compare \
        --max-docs 30
...
def main() -> None:
    if not MAP_PATH.exists():
        raise SystemExit(
            ...
            "run `evals ingest multimodal_doc mmlongbench` first."
        )
