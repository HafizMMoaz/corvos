...
def main() -> None:
    ...
    for qid, arms in by_q.items():
        ...
        s = arms.get("corvos", {}).get("graded", {}).get("grade")
    ...
    print(f"\nCorvos INCORRECT but Long-Context CORRECT: {len(surf_wrong_lc_right)}")
    print(f"Long-Context INCORRECT but Corvos CORRECT: {len(lc_wrong_surf_right)}")
    print(f"Corvos INCORRECT but Bare CORRECT: {len(surf_wrong_bare_right)}")

    print("\n=== Where Corvos is wrong but long-context is right (top 5) ===")
    for qid in surf_wrong_lc_right[:5]:
        arms = by_q[qid]
        b = arms.get("bare_llm", {})
        print(f"\n[{qid}] domain={b.get('domain')} qtype={b.get('question_type')}")
        print(f"  GOLD: {b.get('gold')!r}")
        for arm_name in ("bare_llm", "long_context", "corvos"):
            a = arms.get(arm_name, {})
    ...
    print("\n=== Where Long-Context is wrong but Corvos is right (top 5) ===")
