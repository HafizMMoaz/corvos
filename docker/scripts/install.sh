#!/usr/bin/env bash
# =============================================================================
# Corvos - One-line Install Script
#
#
# Usage: curl -fsSL https://raw.githubusercontent.com/HafizMMoaz/Corvos/main/docker/scripts/install.sh | bash
#
# Flags:
...
REPO_RAW="https://raw.githubusercontent.com/HafizMMoaz/Corvos/main"
INSTALL_DIR="./corvos"
OLD_VOLUME="corvos-data"
DUMP_FILE="./corvos_migration_backup.sql"
KEY_FILE="./corvos_migration_secret.key"
...
info()    { printf "${CYAN}[Corvos]${NC} %s\n"        "$1"; }
success() { printf "${GREEN}[Corvos]${NC} %s\n"       "$1"; }
warn()    { printf "${YELLOW}[Corvos]${NC} %s\n"      "$1"; }
error()   { printf "${RED}[Corvos]${NC} ERROR: %s\n"  "$1" >&2; exit 1; }
step()    { printf "\n${BOLD}${CYAN}── %s${NC}\n"        "$1"; }
...
EOF
    ...
    printf "${CYAN}══════════════════════════════════════════════════════════════${NC}\n"
    info "This installer will create ${INSTALL_DIR}/ and start Corvos with Docker Compose."
}
...
stack_failure_report() {
    ...
    echo "  2. Verify publication:   cd ${INSTALL_DIR} && ${DC} exec db psql -U corvos -d corvos -c 'SELECT pubname FROM pg_publication;'"
    echo "  3. Hard reset zero db:   cd ${INSTALL_DIR} && ${DC} down && docker volume rm corvos-zero-cache && ${DC} up -d --wait"
    echo ""
    exit 1
}
...
resolve_variant() {
    ...
    if $VARIANT_EXPLICIT; then
        ...
        if ! $has_runtime; then
            ...
            warn "Install the toolkit before enabling Corvos GPU acceleration." >&2
            printf 'cpu'
            return 0
        fi
    ...
    if $has_gpu && $has_runtime && ! $QUIET && [[ -r /dev/tty && -w /dev/tty ]]; then
        local choice
        echo "" > /dev/tty
        printf "${BOLD}${CYAN}Corvos detected an NVIDIA GPU.${NC}\n" > /dev/tty
...
apply_variant_env() {
    ...
    if [[ -f "$env_file" && "$allow_existing_update" != "true" ]]; then
        ...
        info "To change variants later, edit CORVOS_VARIANT and COMPOSE_FILE in ${env_file}, then run ${DC} up -d --wait."
        return 0
    fi

    if [[ "$variant" == "cpu" ]]; then
        set_env_value "$env_file" "CORVOS_VARIANT" ""
        remove_env_value "$env_file" "COMPOSE_FILE"
        remove_env_value "$env_file" "CORVOS_GPU_COUNT"
    else
        set_env_value "$env_file" "CORVOS_VARIANT" "$variant"
        set_env_value "$env_file" "COMPOSE_FILE" "docker-compose.yml:docker-compose.gpu.yml"
        if [[ -n "$GPU_COUNT" ]]; then
            set_env_value "$env_file" "CORVOS_GPU_COUNT" "$GPU_COUNT"
        fi
    fi
...
# Detect corvos-data volume → migration mode.
...
if docker volume ls --format '{{.Name}}' 2>/dev/null < /dev/null | grep -q "^${OLD_VOLUME}$" \
   && [[ ! -f "${MIGRATION_DONE_FILE}" ]]; then
    ...
    if [[ -f "${DUMP_FILE}" ]]; then
        ...
        info "Full extraction log: ./corvos-migration.log"
        printf "\n"
...
if [ ! -f "${INSTALL_DIR}/.env" ]; then
    ...
    if $VARIANT_EXPLICIT; then
        ...
        info "Updated Corvos image variant in existing ${INSTALL_DIR}/.env"
    else
        apply_variant_env "${INSTALL_DIR}/.env" "$SELECTED_VARIANT" "false"
    fi
fi
...
if $MIGRATION_MODE; then
    ...
    DB_USER="${DB_USER:-corvos}"
    DB_PASS="${DB_PASS:-corvos}"
    DB_NAME="${DB_NAME:-corvos}"
    ...
    if [[ -n "${FATAL_ERRORS}" ]]; then
        ...
        printf "%s\n" "${FATAL_ERRORS}"
        warn "If Corvos behaves incorrectly, inspect manually:"
    ...
    step "Starting all Corvos services"
...
if $SETUP_WATCHTOWER; then
    ...
    if [[ "${WT_STATE}" == "true" ]]; then
        ...
        docker run -d \
            ...
            && success "Watchtower started - labeled Corvos containers will auto-update." \
...
echo ""
_version_display=$(grep '^CORVOS_VERSION=' "${INSTALL_DIR}/.env" 2>/dev/null | cut -d= -f2 | tr -d '"' | head -1 || true)
_version_display="${_version_display:-latest}"
_variant_display=$(grep '^CORVOS_VARIANT=' "${INSTALL_DIR}/.env" 2>/dev/null | cut -d= -f2 | tr -d '"' | head -1 || true)
_variant_display="${_variant_display:-cpu}"
step "Corvos is now installed [${_version_display}]"

_public_url=$(grep '^CORVOS_PUBLIC_URL=' "${INSTALL_DIR}/.env" 2>/dev/null | cut -d= -f2- | tr -d '"' | head -1 || true)
_public_url="${_public_url:-http://localhost:3929}"

info "  Corvos: ${_public_url}"
