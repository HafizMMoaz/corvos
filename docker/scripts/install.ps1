# =============================================================================
# Corvos - One-line Install Script (Windows / PowerShell)
#
#
# Usage: irm https://raw.githubusercontent.com/HafizMMoaz/Corvos/main/docker/scripts/install.ps1 | iex
#
...
#   1. Fresh install        - no prior Corvos data detected
#   2. Migration from the legacy all-in-one container (corvos-data volume)
...
$RepoRaw            = "https://raw.githubusercontent.com/HafizMMoaz/Corvos/main"
$InstallDir         = ".\corvos"
$OldVolume          = "corvos-data"
$DumpFile           = ".\corvos_migration_backup.sql"
$KeyFile            = ".\corvos_migration_secret.key"
...
if ($Variant -and $Variant -notin @("cpu", "cuda", "cuda126")) {
    Write-Host "[Corvos] ERROR: Invalid -Variant '$Variant'. Use 'cpu', 'cuda', or 'cuda126'." -ForegroundColor Red
    exit 1
}

if ($GpuCount -and $GpuCount -notmatch '^([0-9]+|all)$') {
    Write-Host "[Corvos] ERROR: Invalid -GpuCount '$GpuCount'. Use a number or 'all'." -ForegroundColor Red
    exit 1
}

# ── Output helpers ──────────────────────────────────────────────────────────

function Write-Info    { param([string]$Msg) Write-Host "[Corvos] " -ForegroundColor Cyan -NoNewline; Write-Host $Msg }
function Write-Ok      { param([string]$Msg) Write-Host "[Corvos] " -ForegroundColor Green -NoNewline; Write-Host $Msg }
function Write-Warn    { param([string]$Msg) Write-Host "[Corvos] " -ForegroundColor Yellow -NoNewline; Write-Host $Msg }
function Write-Step    { param([string]$Msg) Write-Host "`n-- $Msg" -ForegroundColor Cyan }
function Write-Err     { param([string]$Msg) Write-Host "[Corvos] ERROR: $Msg" -ForegroundColor Red; exit 1 }
...
"@ -ForegroundColor White
    ...
    Write-Info "This installer will create $InstallDir\ and start Corvos with Docker Compose."
}
...
function Invoke-StackFailureReport {
    ...
    Write-Host "  2. Verify publication:   cd $InstallDir; docker compose exec db psql -U corvos -d corvos -c 'SELECT pubname FROM pg_publication;'"
    Write-Host "  3. Hard reset zero db:   cd $InstallDir; docker compose down; docker volume rm corvos-zero-cache; docker compose up -d --wait"
    Write-Host ""
    exit 1
}
...
function Resolve-Variant {
    ...
    if ($Variant) {
        ...
        if (-not $hasRuntime) {
            ...
            Write-Warn "Install the toolkit before enabling Corvos GPU acceleration."
            return "cpu"
        }
        return $Variant
    }
    ...
    if ($hasGpu -and $hasRuntime -and -not $Quiet -and [Environment]::UserInteractive) {
        Write-Host ""
        Write-Host "Corvos detected an NVIDIA GPU." -ForegroundColor Cyan
...
function Set-VariantEnv {
    ...
    if ((Test-Path $Path) -and -not $AllowExistingUpdate) {
        ...
        Write-Info "To change variants later, edit CORVOS_VARIANT and COMPOSE_FILE in $Path, then run docker compose up -d --wait."
        return
    }

    if ($SelectedVariant -eq "cpu") {
        Set-EnvValue -Path $Path -Key "CORVOS_VARIANT" -Value ""
        Remove-EnvValue -Path $Path -Key "COMPOSE_FILE"
        Remove-EnvValue -Path $Path -Key "CORVOS_GPU_COUNT"
    } else {
        Set-EnvValue -Path $Path -Key "CORVOS_VARIANT" -Value $SelectedVariant
        Set-EnvValue -Path $Path -Key "COMPOSE_FILE" -Value "docker-compose.yml;docker-compose.gpu.yml"
        if ($GpuCount) {
            Set-EnvValue -Path $Path -Key "CORVOS_GPU_COUNT" -Value $GpuCount
        }
    }
...
Write-Step "Downloading Corvos files"
...
if (($volumeList -split "`n") -contains $OldVolume -and -not (Test-Path $MigrationDoneFile)) {
    ...
    if (Test-Path $DumpFile) {
        ...
        Write-Info "Full extraction log: ./corvos-migration.log"
        Write-Host ""

        $migrateScript = Join-Path $InstallDir "scripts/migrate-database.ps1"
        & $migrateScript -Yes
        if ($LASTEXITCODE -ne 0) {
            Write-Err "Data extraction failed. See ./corvos-migration.log for details.`nYou can also run migrate-database.ps1 manually with custom flags."
        }
...
if (-not (Test-Path $envPath)) {
    ...
    if ($PSBoundParameters.ContainsKey('Variant')) {
        ...
        Write-Info "Updated Corvos image variant in existing $envPath"
    } else {
        Set-VariantEnv -Path $envPath -SelectedVariant $SelectedVariant -AllowExistingUpdate $false
    }
}
...
if ($MigrationMode) {
    ...
    if (-not $DbUser) { $DbUser = "corvos" }
    if (-not $DbPass) { $DbPass = "corvos" }
    if (-not $DbName) { $DbName = "corvos" }
    ...
    $restoreErrFile = Join-Path $env:TEMP "corvos_restore_err.log"
    Push-Location $InstallDir
    ...
    if ($fatalErrors.Count -gt 0) {
        ...
        Write-Warn "If Corvos behaves incorrectly, inspect manually."
    } else {
    ...
    Write-Step "Starting all Corvos services"
...
if ($SetupWatchtower) {
    ...
    if ($wtState -eq "true") {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Ok "Watchtower started - labeled Corvos containers will auto-update."
        } else {
...
Write-Host ""
$versionDisplay = (Get-Content $envPath | Select-String '^CORVOS_VERSION=' | ForEach-Object { ($_ -split '=',2)[1].Trim('"') }) | Select-Object -First 1
if (-not $versionDisplay) { $versionDisplay = "latest" }
$variantDisplay = (Get-Content $envPath | Select-String '^CORVOS_VARIANT=' | ForEach-Object { ($_ -split '=',2)[1].Trim('"') }) | Select-Object -First 1
if (-not $variantDisplay) { $variantDisplay = "cpu" }
$wtHours = [math]::Floor($WatchtowerInterval / 3600)
Write-Step "Corvos is now installed [$versionDisplay]"
