#!/usr/bin/env bash
# =============================================================================
# Corvos - Database Migration Script
#
...
#
# Options:
#   --db-user USER        Old PostgreSQL username   (default: corvos)
#   --db-password PASS    Old PostgreSQL password   (default: corvos)
#   --db-name NAME        Old PostgreSQL database   (default: corvos)
...
#   - Install the new Corvos stack (install.sh handles that automatically)
#
# Note:
...
TEMP_CONTAINER="corvos-pg14-migration"
...
confirm() {
    ...
    printf "${YELLOW}[Corvos]${NC} %s [y/N] " "$1"
    read -r reply
...
cleanup() {
    ...
    if [[ $exit_code -ne 0 ]]; then
        printf "\n${RED}[Corvos]${NC} Migration data extraction failed (exit code %s).\n" "${exit_code}" >&2
        printf "${RED}[Corvos]${NC} Full log: %s\n" "${LOG_FILE}" >&2
        printf "${YELLOW}[Corvos]${NC} Your original data in '${OLD_VOLUME}' is untouched.\n" >&2
    fi
}
trap cleanup EXIT
...
docker volume ls --format '{{.Name}}' < /dev/null | grep -q "^${OLD_VOLUME}$" \
    || error "Legacy volume '${OLD_VOLUME}' not found.\n       Are you sure you ran the old all-in-one Corvos container?"
...
if ! docker exec \
        ...
        > "${DUMP_FILE}" 2>/tmp/corvos_pgdump_err < /dev/null; then
    cat /tmp/corvos_pgdump_err >&2
...
if docker run --rm -v "${OLD_VOLUME}:/data" alpine \
    ...
    if $AUTO_YES; then
        ...
        warn "To restore your original key, update SECRET_KEY in ./corvos/.env afterwards."
    else
        printf "${YELLOW}[Corvos]${NC} Enter the SECRET_KEY from your old container's environment\n"
        printf "${YELLOW}[Corvos]${NC} (press Enter to generate a new one - existing sessions will be invalidated): "
...
printf "\n"
printf "${CYAN}  curl -fsSL https://raw.githubusercontent.com/HafizMMoaz/Corvos/main/docker/scripts/install.sh | bash${NC}\n"
printf "\n"
info "install.sh will detect the dump, restore your data into PostgreSQL 17,"
info "and start the full Corvos stack automatically."
printf "\n"
