# =============================================================================
# Corvos - Database Migration Script (Windows / PowerShell)
#
# Extracts data from the legacy all-in-one corvos-data volume (PostgreSQL 14)
...
#
# Options:
#   -DbUser USER          Old PostgreSQL username   (default: corvos)
#   -DbPassword PASS      Old PostgreSQL password   (default: corvos)
#   -DbName NAME          Old PostgreSQL database   (default: corvos)
#   -Yes                  Skip all confirmation prompts
#
# Prerequisites:
#   - Docker Desktop installed and running
#   - The legacy corvos-data volume must exist
#   - ~500 MB free disk space for the dump file
#
# What this script does:
#   1. Stops any container using corvos-data (to prevent corruption)
#   2. Starts a temporary PG14 container against the old volume
#   3. Dumps the database to .\corvos_migration_backup.sql
#   4. Recovers the SECRET_KEY to .\corvos_migration_secret.key
#   5. Exits - leaving installation to install.ps1
#
# What this script does NOT do:
#   - Delete the original corvos-data volume (do this manually after verifying)
#   - Install the new Corvos stack (install.ps1 handles that automatically)
#
# Note:
#   install.ps1 downloads and runs this script automatically when it detects the
#   legacy corvos-data volume. You only need to run this script manually if
...
param(
    [string]$DbUser     = "corvos",
    [string]$DbPassword = "corvos",
    [string]$DbName     = "corvos",
    [switch]$Yes
)
...
function Confirm-Action {
    param([string]$Prompt)
    if ($Yes) { return }
    $reply = Read-Host "[Corvos] $Prompt [y/N]"
...
Register-EngineEvent PowerShell.Exiting -Action {
    $containers = Invoke-NativeSafe { docker ps -a --format '{{.Names}}' 2>$null }
    if ($containers -and ($containers -split "`n") -contains "corvos-pg14-migration") {
        Invoke-NativeSafe { docker stop "corvos-pg14-migration" *>$null } | Out-Null
        Invoke-NativeSafe { docker rm "corvos-pg14-migration" *>$null } | Out-Null
    }
} | Out-Null
...
if (-not (($volumeList -split "`n") -contains $OldVolume)) {
    Write-Err "Legacy volume '$OldVolume' not found. Are you sure you ran the old all-in-one Corvos container?"
}
...
$pgDumpErrFile = Join-Path $env:TEMP "corvos_pgdump_err.log"
...
if ($LASTEXITCODE -eq 0 -and $keyCheck) {
    ...
    if ($Yes) {
        ...
        Write-Warn "To restore your original key, update SECRET_KEY in .\corvos\.env afterwards."
    } else {
        Write-Warn "Enter the SECRET_KEY from your old container's environment"
        $recoveredKey = Read-Host "[Corvos] (press Enter to generate a new one - existing sessions will be invalidated)"
        if (-not $recoveredKey) {
            $bytes = New-Object byte[] 32
            $rng = [System.Security.Cryptography.RNGCryptoServiceProvider]::new()
            $rng.GetBytes($bytes)
...
Write-Host ""
Write-Host "  irm https://raw.githubusercontent.com/HafizMMoaz/Corvos/main/docker/scripts/install.ps1 | iex" -ForegroundColor Cyan
Write-Host ""
Write-Info "install.ps1 will detect the dump, restore your data into PostgreSQL 17,"
Write-Info "and start the full Corvos stack automatically."
Write-Host ""
